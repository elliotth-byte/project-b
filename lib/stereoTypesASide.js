...
// Exported specifically so lib/achievements/evaluate.js can reuse this
// exact scoring logic for its own "Round Perfect"/"Pump Up the Volume"
// achievements (see that file's evaluateStereoTypesAchievements) —
// those need the same per-guess guessResults/pumpedCorrect detail this
// already computes, and re-deriving that from scratch would risk
// drifting out of sync with the real scoring rules above.
export function computeASideScores(state) {
  const perPlayer = {};
