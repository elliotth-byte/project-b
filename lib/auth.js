...
//
// The explicit refreshSession() after is deliberate, not redundant:
// sql/add-host-scope.sql's games-insert RLS policy reads role/hostScope
// straight off the request's JWT, not a fresh table lookup — so a
// "Create Season" click immediately after this would otherwise risk
// hitting RLS with whatever JWT was already cached client-side, which
// may or may not yet reflect the metadata this call just wrote,
// depending on exactly when the client's own session was last issued.
// Forcing a refresh here removes that ambiguity outright rather than
// relying on it happening to work out.
//
