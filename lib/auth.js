...
export async function signUpPlayer(username, password) {
  ...
  const { data, error } = await supabase.auth.signUp({
    email,
    password,
    // accepted_terms_at is written here, not as a separate table/write
    // — same pattern already used for role/hostScope (see
    // signInHostFlexible and sql/add-host-scope.sql) for small,
    // durable-enough account flags that don't need their own row.
    // pages/signup.jsx requires the checkbox this comes from before
    // this function is ever called at all.
    options: { data: { display_name: username.trim(), accepted_terms_at: new Date().toISOString() } },
  });
...
export async function signUpHost(email, password, hostScope = null) {
  const { data, error } = await supabase.auth.signUp({
    email,
    password,
    // accepted_terms_at — same reasoning as signUpPlayer's own version
    // of this field just above. pages/login.jsx's host-signup form
    // requires the same checkbox before this is ever called.
    options: { data: { ...(hostScope ? { role: "host", hostScope } : { role: "host" }), accepted_terms_at: new Date().toISOString() } },
  });
  if (error) return { ok: false, error: error.message };
  return { ok: true, user: data.user };
}
...
// pages/host.jsx's own login form used to call signInHost directly,
// which only ever works for an account with a real email — exactly
// right for a dashboard-created or signUpHost-created host account, but
// silently wrong for anyone who got host access via becomeHost(): those
// are still ordinary player accounts underneath (a fake
// username@players.projectb.game email, invisible to the person typing
// their username), so typing that username straight into a field
// literally sent to Supabase as an email fails outright. This is the
// fix: try it as a real email first (so a genuine host account's own
// real email — which could coincidentally contain no "@" typo-wise, but
// realistically always will — still goes straight through), and if that
// specific attempt fails, retry once treating the same input as a
// username via the exact conversion signInPlayer already uses. Two
// login attempts in the failure case is a fine cost for removing a
// confusion this real — someone who just became a host has no way to
// know their account "secretly" has a fake email at all.
export async function signInHostFlexible(identifier, password) {
  const trimmed = identifier.trim();
  const first = await supabase.auth.signInWithPassword({ email: trimmed, password });
  if (!first.error) return { ok: true, user: first.data.user };

  let usernameEmail;
  try {
    usernameEmail = usernameToEmail(trimmed);
  } catch {
    return { ok: false, error: "Wrong email/username or password." };
  }
  const second = await supabase.auth.signInWithPassword({ email: usernameEmail, password });
  if (second.error) return { ok: false, error: "Wrong email/username or password." };
  return { ok: true, user: second.data.user };
}
