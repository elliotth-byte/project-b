...
const CSV_COLUMNS = [
  ...
  ["reason", "Reason"], ["chaosHolder", "Favor of the Fates"], ["nullified", "Nullified?"],
...
// Favor of the Fates (that round's holder), then Exiled (the outcome) right
...
export function buildVotingGrid(exileHistory, finaleState, players, challengeHistory) {
  ...
  // Column key -> { voterId: [{ target, nullified }, ...] } — an ARRAY
  // per voter, not a single object, because Apollo's character power
  // (see lib/characterPowers.js's expandVoteRows) can put two rows in
  // e.voteRows for the same voterId in the same round: his one real
  // vote entry gets expanded into two ordinary rows right before
  // tallying, specifically so the counting logic never has to know
  // either came from the same person. That's exactly right for a tally,
  // but this map used to do `map[v.voterId] = {...}` — a plain
  // overwrite — which meant whichever of his two rows got iterated
  // second silently replaced the first, so his cell only ever showed
  // one of his two votes. nullified means the Favor of the Fates holder
  // zeroed out every vote cast for that target that round (not just one
  // voter's), so it's still keyed off each row's own target, not the
  // voter.
  const votesByColumn = {};
  rounds.forEach((e) => {
    const map = {};
    (e.voteRows || []).forEach((v) => {
      (map[v.voterId] ||= []).push({ target: byId[v.targetId] || v.targetId || "?", nullified: !!e.nullifiedId && v.targetId === e.nullifiedId });
    });
  ...
  if (hasFinale) {
    const map = {};
    (finaleState.voteRows || []).forEach((v) => {
      (map[v.voterId] ||= []).push({ target: byId[v.targetId] || v.targetId || "?", nullified: !!finaleState.nullifiedFinalistId && v.targetId === finaleState.nullifiedFinalistId });
    });
  ...
  // ID-keyed counterparts to winnerByColumn/nomineesByColumn (which are
  // both display-name arrays, built for the header rows above the
  // grid) — used below to shade an individual PLAYER's cell when they
  // themselves were the nominee/immune that round, which needs the
  // actual id, not the name string those header rows already have.
  const nomineeIdsByColumn = {};
  const immuneIdByColumn = {};
  // The target whose votes got zeroed out by the Favor of the Fates
  // that round, if any — used below (alongside nomineeIdsByColumn) to
  // shade a nominee's cell purple when they were saved this way,
  // regardless of whether they personally cast a vote that round.
  const nullifiedIdByColumn = {};
  ...
  rounds.forEach((e) => {
    ...
    immuneIdByColumn[key] = challengeWinnerId || null;
    nullifiedIdByColumn[key] = e.nullifiedId || null;
    ...
    nomineeIdsByColumn[key] = new Set((e.nominees || []).map((n) => n.playerId));
    ...
    // Favor of the Fates that round (see lib/exileLogic.js) — tieBreakChoiceId
    ...
    exiledByColumn[key] = (e.exiledIds || []).map((id) => {
      ...
      if (id === e.tieBreakChoiceId) return `${name} (tie broken by ${byId[e.chaosHolderId] || "the Favor of the Fates holder"})`;
      return name;
    });
  });
  if (hasFinale) {
    winnerByColumn.finale = [];
    immuneIdByColumn.finale = null; // no "immunity" concept in the Finale — jurors vote among finalists, nobody's exempt from being voted for
    nullifiedIdByColumn.finale = finaleState.nullifiedFinalistId || null;
    fatesWinnersByColumn.finale = [];
    nomineesByColumn.finale = (finaleState.finalists || []).map((f) => f.name || byId[f.playerId] || f.playerId);
    nomineeIdsByColumn.finale = new Set(); // deliberately empty — see immuneIdByColumn.finale's own comment; "nominee" here would really mean "finalist", a different status this shading isn't meant to represent
    ...
    if (finaleState.winnerId) {
      ...
      exiledByColumn.finale = [
        finaleState.winnerId === finaleState.tieBreakChoiceId
          ? `${winnerName} wins (tie broken by ${byId[finaleState.chaosHolderId] || "the Favor of the Fates holder"})`
          : `${winnerName} wins`,
      ];
    } else {
      exiledByColumn.finale = [];
  ...
  // actually comes from.
  // Runner-up/3rd, straight from finale.placements (see
  // lib/exileLogic.js's computeFinaleOutcome, stored once at reveal by
  // lib/roundEngine.js's advanceFromFinale) — the exact same source the
  // in-app "🥈 X finishes 2nd. 🥉 Y finishes 3rd." reveal message already
  // uses, so this label always agrees with that announcement. Only
  // meaningful once there's a winner (placements is null/empty until
  // then), same as the Winner check right below.
  const placementLabelById = {};
  if (hasFinale) {
    (finaleState.placements || []).forEach((p) => {
      if (p.place === 2) placementLabelById[p.playerId] = { label: "Runner-Up", color: "#c0c0c0" };
      else if (p.place === 3) placementLabelById[p.playerId] = { label: "3rd Place", color: "#cd7f32" };
    });
  }

  const statusFor = (id) => {
    if (hasFinale && finaleState.winnerId === id) return { label: "Winner", color: "#00ff9d" };
    if (placementLabelById[id]) return placementLabelById[id];
  ...
  const playerRows = orderedIds.map((id) => ({
    ...
    status: statusFor(id),
    // nominated/immune/savedByFates shade the cell itself — this
    // player's own status in the SAME round their vote is shown for,
    // not a general "were they ever nominated" flag. Deliberately
    // computed independent of whether they actually cast a vote that
    // round (a nominee who didn't vote — sitting out, already at their
    // vote limit, whatever the reason — still gets shaded), so every
    // cell always carries these flags; only `votes` (each entry's own
    // `nullified`, which only ever applies to that specific vote) is an
    // empty array when there's nothing to show. savedByFates takes
    // priority in the component's own color choice below over plain
    // "nominated" — being nominated but having your votes zeroed out is
    // a meaningfully different outcome from an ordinary nomination.
    //
    // `votes` is always an ARRAY (usually 0 or 1 entries, occasionally 2
    // for an Apollo double vote — see votesByColumn above) rather than a
    // single nullable `target`/`nullified` pair, so a consumer that only
    // wants "the" vote can still just read votes[0], but the grid
    // component below renders every entry.
    cells: columns.map((c) => {
      const votes = votesByColumn[c.key]?.[id] ?? [];
      const nominated = nomineeIdsByColumn[c.key]?.has(id) || false;
      const immune = immuneIdByColumn[c.key] === id;
      const savedByFates = nominated && nullifiedIdByColumn[c.key] === id;
      return { votes, nominated, immune, savedByFates };
    }),
  }));
...
export function votingGridToCSV(grid) {
  ...
  const lines = grid.playerRows.map((r) => [
    ...
    // Joined with " / " for anyone with two votes that round (Apollo's
    // character power) rather than dropping the second — see cells'
    // own `votes` array in buildVotingGrid above.
    ...r.cells.map((c) => (c?.votes || []).map((v) => v.target + (v.nullified ? " (nullified)" : "")).join(" / ")),
  ].map(csvEscape).join(","));
  return [
    header,
    ...
    headerRow("Favor of the Fates", grid.chaosByColumn),
