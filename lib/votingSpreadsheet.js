...
export function buildVotingGrid(exileHistory, finaleState, players, challengeHistory) {
  ...
  const playerRows = orderedIds.map((id) => ({
    ...
    status: statusFor(id),
    // nominated/immune shade the cell itself — this player's own status
    // in the SAME round their vote is shown for, not a general "were
    // they ever nominated" flag. Only meaningful (and only ever set)
    // on a cell that actually has a vote in it — a nominee/immune
    // player who didn't vote that round has nothing here to shade in
    // the first place (cell is already null).
    cells: columns.map((c) => {
      const vote = votesByColumn[c.key]?.[id] ?? null;
      if (!vote) return null;
      return { ...vote, nominated: nomineeIdsByColumn[c.key]?.has(id) || false, immune: immuneIdByColumn[c.key] === id };
    }),
  }));
