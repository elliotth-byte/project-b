...
export async function sendGroupMessage(gameId, senderId, senderName, body) {
  const text = (body || "").trim();
  if (!text) return false;
  const res = await storageUpdate(gameId, GROUP_CHAT_KEY, (fresh) => {
    const list = fresh || [];
    const next = [...list, { id: `${Date.now()}-${Math.random().toString(36).slice(2)}`, senderId, senderName, body: text, createdAt: Date.now() }];
...
export async function getOrCreateThread(gameId, myPlayerId, otherPlayerId) {
  ...
  if (error) {
    // Lost a race with the other participant opening the same thread at
    // the same instant — just fetch what they created instead of erroring.
