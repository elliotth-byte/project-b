...
async function autoStartRandomChallenge(env) {
  ...
  const now = Date.now();
  // torched was removed from this infinite-duration list — it now runs
  // for the season's own configured challengeDurationSec like any other
  // challenge (see autoTimeoutTorched's own comment on why a per-turn
  // timeout was added alongside this, not on its own — an unbounded
  // per-turn wait inside a now-bounded battle would just mean one
  // inactive player could burn the whole clock doing nothing).
  const endsAt = (settings.infiniteTime || gameType === "masquerade" || gameType === "chains" || gameType === "pandorasboxes") ? null : now + (settings.challengeDurationSec || 900) * 1000;
