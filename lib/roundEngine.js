...
import { logGameEvent } from "./debugLog";
...
async function healNominatorOrder(env, fates) {
  const { gameId, db, alivePlayers, postMessage } = env;
  ...
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.placements) return fates; // nothing to fall back to — leave it for a human or the timeout
  ...
  const fatesRes = await db.update(gameId, KEY_FATES, (fresh) => {
    ...
    fresh.nominatorOrder.forEach((outgoing) => {
      ...
      if (!replacement) return; // no one left to promote
