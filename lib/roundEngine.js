...
async function advanceFromExile(env) {
  ...
  if (remainingAlive.length <= 3) {
    ...
    await postMessage(
      `🔥 We've reached the Finale! Finalists: ${remainingAlive.map((p) => p.display_name).join(", ")}.\n\nFinalists can write their statement, and the jury can ask questions — every finalist can respond. Once that's done, the host opens voting: every exiled player who didn't quit or wasn't removed votes for a winner, and the Power of Chaos is up for grabs among them too.`
    );
...
async function advanceFromFinale(env) {
  ...
  await postMessage(
    ...
    (thirdName ? `\n🥉 ${thirdName} finishes 3rd${nullifiedId === third?.playerId ? " — nullified by the Power of Chaos" : ""}.` : "")
  );
  return { advanced: true, to: PHASES.ENDED };
}
