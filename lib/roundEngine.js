...
async function advanceFromExile(env) {
  ...
  if (!exile.revealed) {
    for (const id of exiledIds) {
      ...
      // block an actual exile from finalizing. try/catch here isn't
      // redundant with checking `error` below — an RPC call to a
      // function that doesn't exist yet can throw outright rather than
      // resolving with an error object, and this whole loop runs BEFORE
      // the history/ceremony data gets written a few lines down, so an
      // uncaught throw here previously meant: player marked exiled for
      // real, then the function aborts before ever recording the round's
      // history — a real exile with no ceremony or voting-history entry
      // to show for it.
      try {
    ...
    const historyRes = await db.update(gameId, KEY_EXILE_HISTORY, (fresh) => {
      ...
      return [...list, {
        ...
        voteRows,
        // Computed ONCE here, not by whoever's viewing — buildRevealOrder
        // uses real randomness, so if every player's screen (and the
        // host's) computed this itself, everyone would see a DIFFERENT
        // vote-by-vote order. Storing it is what makes it one shared,
        // consistent moment instead of each viewer getting their own
        // (mismatched) version of what happened.
