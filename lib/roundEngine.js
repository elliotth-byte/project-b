...
import { evaluateAndAwardAchievements } from "./achievements/evaluate";
import { logGameEvent, summarize } from "./debugLog";
import { scoresToPlacements } from "./challengeScores";
import { applyZeusPower, computeAresImmunityUpdates, overrideStatusForHades, filterCancelledVote, expandVoteRows, powerFor, aphroditeBlocksTargeting, findAresImmunePlayerId } from "./characterPowers";
import { pickRandomChallenge, pickHephaestusOptions, hephaestusDrawKey, randomPickKey } from "./challengeSelection";
import { markGameTypesUnlocked } from "./unlockedGames";
import { presetIncompleteGameTypes } from "./presetReadiness";
import { bigScreenOnlyGameTypesToExclude } from "./bigScreenOnlyGames";
...
import { computeEliminateOutcome, computeSaveOutcome, computeFinaleOutcome, buildRevealOrder, resolveEliminateTieByChallenge, pickRandomAmong } from "./exileLogic";
import { REENTRY_STATUS, resolveReentryAttempt } from "./reentryLogic";
import { consumePreDecidedReentryIns } from "./reentryData";
import { sendPushToGame, sendPushToHosts } from "./sendPush";
import { GROUP_CHAT_KEY } from "./chatData";
import { sendNativePushToGame } from "./sendNativePush";
...
import { initWordScrambleTv } from "./games/wordScrambleTvData";
import { initSimonTv, resolveSimonRound, simonTvKey, placementValue as simonTvPlacementValue } from "./games/simonTvData";
import { initMusicalChairsTv, resolveMusicalChairsRound, musicalChairsTvKey, placementValue as musicalChairsTvPlacementValue } from "./games/musicalChairsTvData";
import { initEyesInTheSystem } from "./games/eyesInTheSystemData";
import { initEyesInTheSystemTv, resolveEyesTvMatch, eyesTvKey, placementValue as eyesTvPlacementValue } from "./games/eyesInTheSystemTvData";
import { initBalloono, resolveExplosions as resolveBalloonoExplosions, tickBubbles as tickBalloonoBubbles, balloonoKey, placementValue as balloonoPlacementValue } from "./games/balloonoData";
import { initLaurelThief } from "./games/laurelThiefData";
import { initWagerTrivia, tickWagerTrivia } from "./games/wagerTriviaTvData";
import { initTartarusTreadmill, tickTartarusTreadmill, tartarusTreadmillKey, placementValue as tartarusTreadmillPlacementValue } from "./games/tartarusTreadmillData";
import { initSpyfall, tickSpyfall, spyfallKey, placementValue as spyfallPlacementValue } from "./games/spyfallData";
import { initAcrophobia, tickAcrophobia } from "./games/acrophobiaData";
import { initMiniGolf, tickMiniGolf, miniGolfKey, placementValue as miniGolfPlacementValue } from "./games/miniGolfData";
import { initCloseToTwenty } from "./games/closeToTwentyData";
import { initTorched, startShootingPhase, resolveRound as resolveTorchedRound, placementValue as torchedPlacementValue, torchedKey } from "./games/torchedData";
import { isShielded, nextStrikeState, isStrikeDecayRound, decayedStrikeCount, meetsInstantRemovalCriteria } from "./inactivity";
import { recordElimination, clearElimination } from "./seasonPlacement";
...
import { initPandorasBoxes, placementValue as pandorasBoxesPlacementValue, pandorasBoxesKey } from "./games/pandorasBoxesData";
import { initMusicalChairs, advanceMusicalChairsIfDue, finalizeOnTimeout as finalizeMusicalChairsOnTimeout, placementValue as musicalChairsPlacementValue, musicalChairsKey } from "./games/musicalChairsData";
import { initFloor, autoChooseFloorOpponentIfDue, autoResolveFloorDuelIfDue, finalizeFloorOnTimeout, placementValue as floorPlacementValue, floorKey } from "./games/floorData";
import { initArtAuction, autoOpenBiddingIfDue, autoResolveAuctionIfDue, finalizeArtAuctionOnTimeout, placementValue as artAuctionPlacementValue, artAuctionKey } from "./games/artAuctionData";
import { initMysteryButton, autoPassIfDue as autoPassMysteryButtonIfDue, finalizeMysteryButtonOnTimeout, placementValue as mysteryButtonPlacementValue, mysteryButtonKey } from "./games/mysteryButtonData";
import { tickGoldenFleece, placementValue as goldenFleecePlacementValue, goldenFleeceKey } from "./games/goldenFleeceData";
import { tickTriggerHappyTv } from "./games/triggerHappyTvData";
import { tickRiverStyx, placementValue as riverStyxPlacementValue, riverStyxKey } from "./games/riverStyxData";
import { tickWineDarkSea, placementValue as wineDarkSeaPlacementValue, wineDarkSeaKey } from "./games/wineDarkSeaData";
import { tickMajorityRules, majorityRulesKey } from "./games/majorityRulesData";
import { tickMajorityRulesTv, placementValue as majorityRulesTvPlacementValue, majorityRulesTvKey } from "./games/majorityRulesTvData";
import { tickGodsAndGambits, placementValue as godsAndGambitsPlacementValue, godsAndGambitsKey } from "./games/godsAndGambitsData";
import { tickDivinersDice, placementValue as divinersDicePlacementValue, divinersDiceKey } from "./games/divinersDiceData";
import { initSplitFriction, tickSplitFriction, finalizeSplitFrictionOnTimeout, placementValue as splitFrictionPlacementValue, splitFrictionKey } from "./games/splitFrictionData";
import { initCrowns, tickCrowns, placementValue as crownsPlacementValue, crownsKey } from "./games/crownsData";
import { initPoseidonsPool, tickPoseidonsPool, placementValue as poseidonsPoolPlacementValue, poolKey as poseidonsPoolKey } from "./games/poseidonsPoolData";
import { initMidasHoard, tickMidasHoard, midasHoardKey, placementValue as midasHoardPlacementValue } from "./games/midasHoardData";
import { initChariots, tickChariots } from "./games/chariotsData";
import { initScyllasStrait, tickScyllasStrait, scyllasStraitKey, placementValue as scyllasStraitPlacementValue } from "./games/scyllasStraitData";
import {
...
async function notifyRoundChange(gameId, title, body) {
  try {
    ...
    await sendNativePushToGame(gameId, { title, body, url: `/play?game=${gameId}`, tag: "round-change", filterColumn: "notify_rounds" });
  } catch (e) {
...
// A real group chat message, not just an Announcements-feed entry —
// see lib/announcements.js's own header comment on what postMessage
// (makeInAppPostMessage) actually is: a passive, in-app-only feed that
// never triggers a push notification at all, a leftover from when
// these used to post to GroupMe automatically. That's genuinely too
// easy to miss for the one event this exists for: a player being
// permanently removed for inactivity is a big enough deal that the
// rest of the room should actually get pinged about it, the same way
// they would for anyone sending a real message — not just see a line
// appear in a feed tab nobody's necessarily looking at. Writes
// directly to the same GROUP_CHAT_KEY components/ChatPanel.jsx reads
// from (using this file's own server-side db adapter, not
// lib/chatData.js's sendGroupMessage, which is browser-only — it
// calls supabase.auth.getSession() directly, which doesn't exist in
// this server-side context), then sends both push channels the exact
// way a normal chat message does. aliveOnly: true for the same reason
// pages/api/push/notify-message.js's own group-chat branch already
// uses it — and it also correctly excludes the player who was just
// removed, since they're no longer alive either.
async function postSystemChatMessage(env, text) {
  const { db, gameId } = env;
  try {
    await db.update(gameId, GROUP_CHAT_KEY, (fresh) => {
      const list = fresh || [];
      return [...list, { id: `system-${Date.now()}-${Math.random().toString(36).slice(2)}`, senderId: "system", senderName: "⚙️ System", senderRealName: "⚙️ System", body: text, createdAt: Date.now() }];
    });
  } catch (e) {
    console.error("System chat message write failed:", e);
  }
  const title = "⚙️ System";
  try {
    await sendPushToGame(gameId, { title, body: text, url: `/play?game=${gameId}`, tag: "chat-group", filterColumn: "notify_public_messages", aliveOnly: true });
    await sendNativePushToGame(gameId, { title, body: text, url: `/play?game=${gameId}`, tag: "chat-group", filterColumn: "notify_public_messages", aliveOnly: true });
  } catch (e) {
    console.error("System chat message push notify failed:", e);
  }
}
...
async function applyInactivityStrike(env, playerId, reason) {
  ...
  if (actuallyRemoved) await recordElimination(client, gameId, playerId);

  // Every strike gets its own log entry regardless of triggerReason —
  // this is exactly the record that was missing when a real season saw
  // players wrongly struck for "missing" a vote that, by every
  // configured setting, was still open (see
  // sql/add-debug-log.sql's own header for that full story). A host
  // questioning a strike now has a timestamped answer for exactly when
  // it fired and why, instead of needing to reconstruct it after the
  // fact from source code.
  logGameEvent(client, gameId, {
    event: "inactivity_strike",
    round: round?.round ?? null,
    source: "auto",
    detail: { playerId, triggerReason: reason, newStrikes, removed: actuallyRemoved },
  });
...
async function checkInstantInactivityRemoval(env, endedRound, endedRoundStartedAt, playersAliveAfterOutcome) {
  ...
  for (const p of eligible) {
    ...
    await recordElimination(client, gameId, p.id);
    removedNames.push(playersById[p.id] || p.display_name || "someone");
  }

  if (removedNames.length > 0) {
    const msg = `🚫 ${removedNames.join(", ")} didn't vote, play in the Battle, or send any message this round — removed for inactivity.`;
    await postMessage(msg);
    await postSystemChatMessage(env, msg);
  }
}
...
// Fixes a nominatorOrder that was built correctly at the time but has
// since gone stale — specifically the class of bug computeChallengeOutcome's
// own pre-filtering (see advanceFromChallenge's own comment on
// losingReentrantIds) now prevents from happening going forward, but
// can't retroactively fix for a Fates Ceremony that was already
// created before that fix existed. Rather than leave an already-stuck
// ceremony waiting on a manual host override or the full
// fatesDurationSec timeout, this runs the same "who's actually still
// in the game" check every poll and self-heals the order in place:
// any nominator who hasn't submitted yet and is no longer alive gets
// swapped for whoever placed next in the ORIGINAL battle results
// (excluding anyone already in the order, and anyone who's also no
// longer alive) — Dionysus stepping into a vacated 3rd-place slot,
// concretely.
//
// Deliberately narrow in what it touches: a nominator who's ALREADY
// submitted stays exactly as they are regardless of what happens to
// them afterward — their nomination was valid when they made it, and
// this has no business relitigating it. And this only ever promotes
// someone who's actually alive right now; if there's truly no one
// left to promote (every remaining placed player is also gone), it
// leaves that slot alone for the timeout fallback to eventually cover
// — the same honest "nothing left to auto-resolve" posture the rest
// of this file already takes elsewhere rather than fabricating a
// nominator out of nothing.
async function healNominatorOrder(env, fates) {
  const { gameId, db, client, alivePlayers, postMessage, round } = env;
  try {
    const aliveIds = new Set(alivePlayers.map((p) => p.id));

    const needsReplacement = fates.nominatorOrder.filter(
      (n) => !fates.nominations?.[n.playerId] && !aliveIds.has(n.playerId)
    );
    if (needsReplacement.length === 0) return fates; // the normal case, every single poll while a ceremony is otherwise healthy — deliberately silent, no log line, see this function's own history: an earlier version of this comment logged this unconditionally (useful for a few hours while confirming the fix actually ran at all), which turned out to flood the Debug Log's own "last N noteworthy events" window with a repeat-every-10-seconds no-op once the real issue was already fixed. Only the genuinely unusual branches below (something to actually fix, or fixed) still log.

    const challenge = await db.get(gameId, KEY_CHALLENGE);
    if (!challenge?.placements || challenge.placements.length === 0) {
      await logGameEvent(client, gameId, {
        event: "fates_heal_no_challenge_data", round: round?.round, phase: "fates", source: "healNominatorOrder",
        detail: summarize({ needsReplacement: needsReplacement.map((n) => n.name), hasChallenge: !!challenge, placementsCount: challenge?.placements?.length || 0 }),
      });
      return fates; // nothing to fall back to — leave it for a human or the timeout
    }

    const swaps = [];
    const noReplacementFor = [];
    const fatesRes = await db.update(gameId, KEY_FATES, (fresh) => {
      if (!fresh || fresh.round !== fates.round) return fresh;
      const rankedPlacements = rankPlacements(challenge.placements);
      let nextOrder = [...fresh.nominatorOrder];
      fresh.nominatorOrder.forEach((outgoing) => {
        if (fresh.nominations?.[outgoing.playerId]) return; // already nominated by the time this write actually lands — leave them
        if (aliveIds.has(outgoing.playerId)) return; // still alive after all — nothing to fix
        const currentIds = new Set(nextOrder.map((n) => n.playerId));
        const replacement = rankedPlacements.find((p) => !currentIds.has(p.playerId) && aliveIds.has(p.playerId));
        if (!replacement) { noReplacementFor.push(outgoing.name); return; }
        nextOrder = nextOrder.map((n) => (n.playerId === outgoing.playerId ? { playerId: replacement.playerId, name: replacement.name, place: replacement.place } : n));
        swaps.push({ outName: outgoing.name, inName: replacement.name });
      });
      if (swaps.length === 0) return fresh;
      return { ...fresh, nominatorOrder: nextOrder };
    });

    if (noReplacementFor.length > 0) {
      await logGameEvent(client, gameId, {
        event: "fates_heal_no_replacement_found", round: round?.round, phase: "fates", source: "healNominatorOrder",
        detail: summarize({ stuckPlayers: noReplacementFor, placementsCount: challenge.placements.length, aliveCount: aliveIds.size }),
      });
    }
    if (!fatesRes.ok || !fatesRes.value || swaps.length === 0) return fates;

    await logGameEvent(client, gameId, {
      event: "fates_heal_swap_applied", round: round?.round, phase: "fates", source: "healNominatorOrder",
      detail: summarize({ swaps }),
    });
    await postMessage(
      `⚖️ A correction to this Fates Ceremony: ${swaps.map((s) => `${s.outName} → ${s.inName}`).join(", ")} — the outgoing nominator${swaps.length > 1 ? "s are" : " is"} no longer in the game, so the next-placed finisher from the Battle steps into their nominating slot.`
    );
    return fatesRes.value;
  } catch (err) {
    // A bug in this self-healing check should never be able to take
    // the entire Fates Ceremony down with it — that would be strictly
    // worse than the stuck-nominator problem this exists to fix in the
    // first place. Logged so it's actually visible (Debug Log tab)
    // rather than silently swallowed, but this function degrading to a
    // no-op on its own failure is the correct, safe fallback: whatever
    // was stuck stays exactly as stuck as before, not more broken.
    await logGameEvent(client, gameId, {
      event: "fates_heal_error", round: round?.round, phase: "fates", source: "healNominatorOrder",
      detail: summarize({ message: err?.message || String(err) }),
    });
    return fates;
  }
}
...
async function autoNominateTimedOutNominators(env, fates) {
  ...
  if (removedNames.length > 0) {
    ...
    await postSystemChatMessage(env, `🚫 ${removedNames.join(", ")} reached 3 inactivity strikes and ${removedNames.length > 1 ? "have" : "has"} been removed from the game.`);
  }
...
// nominations, or a tie needing the Favor of the Fates holder's call), this
...
// ─── Debug log wiring for advancePhase ───
// See lib/debugLog.js for the full reasoning and sql/add-debug-log.sql
// for the table. Deliberately allowlists which BLOCKED reasons are
// routine (skip logging) rather than which ones are interesting
// (log) — a reason this list doesn't already know about defaults to
// getting logged, which is the safer failure mode for a diagnostics
// tool: a little extra noise from a newly-added reason nobody's
// classified yet, rather than silently missing whatever the NEXT
// unexpected bug's own telltale "blocked" reason turns out to be.
// Every genuine advance (result.advanced === true) always logs,
// regardless of reason.
const ROUTINE_BLOCKED_REASONS = new Set([
  "not-in-progress", "timer-not-up", "voting-still-open", "auto-advance-disabled",
  "no-active-challenge", "no-exile-state", "no-finale-state", "finale-not-set-up-yet",
]);

function logPhaseOutcome(gameId, client, force, round, result, triggeredBy) {
  const noteworthy = result?.advanced || (result?.reason && !ROUTINE_BLOCKED_REASONS.has(result.reason));
  if (noteworthy) {
    // Fire-and-forget, not awaited — logging must never add latency to
    // the actual phase-advance response (see logGameEvent's own
    // header comment on why a logging failure can never be allowed to
    // affect game logic either).
    logGameEvent(client, gameId, {
      event: result.advanced ? "phase_advance" : "phase_blocked",
      round: round?.round ?? null,
      phase: round?.phase ?? null,
      source: force ? "host_force" : "auto",
      detail: { to: result.to || null, reason: result.reason || null, triggeredBy: force ? triggeredBy : null },
    });
  }
  return result;
}

// ─── Host tool: apply a real timer to an already-running challenge ───
// A challenge's endsAt is computed once, the moment it starts, and
// never recomputed after that — so a season's own duration setting
// changing (or a challenge being freshly excluded from the "runs
// forever" list, the way torched/masquerade were) never retroactively
// reaches a battle that was already in progress when that change
// happened. That's expected, not a bug — but it does leave a real
// currently-running battle permanently stuck with whatever endsAt it
// started with, with no way to pick up the new behavior short of
// waiting for it to finish and a fresh one to start. This is that way:
// a host-triggered one-time patch, not a recurring mechanism.
//
// Deliberately does nothing if settings.infiniteTime is on — that's a
// deliberate, current host choice for the WHOLE season, not something
// this should override — and does nothing if the active challenge
// already has a real endsAt, so calling this on a normal, correctly-
// timed battle is always a harmless no-op rather than something a host
// needs to be careful about reaching for.
export async function applyTimerToRunningChallenge(gameId, ctx) {
  const { db, settings } = ctx;
  if (settings.infiniteTime) return { ok: false, reason: "infinite-time-setting-on" };
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.endsAt) return { ok: false, reason: "nothing-to-apply" };

  const endsAt = Date.now() + (settings.challengeDurationSec || 900) * 1000;
  await db.update(gameId, KEY_CHALLENGE, (fresh) => (fresh?.active && !fresh.endsAt ? { ...fresh, endsAt } : fresh));
  await db.update(gameId, KEY_ROUND, (fresh) => (fresh ? { ...fresh, phaseEndsAt: endsAt } : fresh));
  return { ok: true, endsAt };
}

export async function advancePhase(gameId, ctx) {
  const { db, client, postMessage, force = false, now = Date.now(), triggeredBy = null } = ctx;
  ...
  // Self-healing check, same idea and same reasoning as
  // healNominatorOrder further down (see that function's own comment)
  // but for a different contradiction: a re-entry list entry (see
  // lib/reentryLogic.js) claiming status RETURNED — which is only ever
  // supposed to mean "currently back in the game, alive" — for a
  // player who is NOT currently alive. That combination should be
  // structurally impossible through the normal exile/re-entry flow,
  // but a real, reported case showed how it can still happen: a host
  // manually correcting an erroneous elimination (via Roster &
  // Resets) sets the player alive again the same way a genuine
  // re-entry win does, which — before this session's earlier
  // "re-entry counter resets on being exiled again" fix existed —
  // left their entry permanently stuck as RETURNED even after they
  // were legitimately voted out again later, since nothing before that
  // fix ever revisited an existing entry. This runs on every poll,
  // regardless of phase (unlike healNominatorOrder, which only matters
  // during Fates), because there's no round-specific moment tied to
  // catching it — a player can be sitting in this contradictory state
  // for however many rounds have passed since it happened.
  try {
    const reentryList = (await db.get(gameId, KEY_REENTRY)) || [];
    const aliveIds = new Set(alivePlayers.map((p) => p.id));
    const stale = reentryList.filter((r) => r.status === REENTRY_STATUS.RETURNED && !aliveIds.has(r.playerId));
    if (stale.length > 0) {
      const exileHistory = (await db.get(gameId, KEY_EXILE_HISTORY)) || [];
      const latestExileRoundFor = (playerId) => {
        const rounds = exileHistory.filter((e) => (e.exiledIds || []).includes(playerId)).map((e) => e.round);
        return rounds.length > 0 ? Math.max(...rounds) : null;
      };
      const fixedNames = [];
      const listRes = await db.update(gameId, KEY_REENTRY, (fresh) => {
        if (!fresh) return fresh;
        let changed = false;
        const next = fresh.map((r) => {
          if (r.status !== REENTRY_STATUS.RETURNED || aliveIds.has(r.playerId)) return r;
          changed = true;
          fixedNames.push(r.name);
          // Corrected to whatever their actual most recent exile round
          // was, per the exile history itself, rather than left at
          // whatever stale round number the bad entry already had —
          // best-effort: falls back to leaving the existing value
          // alone only if the exile history genuinely has no record of
          // them ever being exiled at all, which shouldn't be
          // reachable given they're already confirmed not alive.
          return { ...r, status: REENTRY_STATUS.PENDING, exiledRound: latestExileRoundFor(r.playerId) ?? r.exiledRound };
        });
        return changed ? next : fresh;
      });
      if (listRes.ok && fixedNames.length > 0) {
        await logGameEvent(client, gameId, {
          event: "reentry_heal_applied", source: "healStaleReturnedReentries",
          detail: summarize({ fixed: fixedNames }),
        });
        await postMessage(
          `🔥 A correction: ${fixedNames.join(", ")} had a re-entry status left over from an earlier host correction that no longer matched their current status — reset to a fresh re-entry attempt.`
        );
      }
    }
  } catch (err) {
    await logGameEvent(client, gameId, {
      event: "reentry_heal_error", source: "healStaleReturnedReentries",
      detail: summarize({ message: err?.message || String(err) }),
    });
  }
  ...
  if (round.phase === PHASES.CHALLENGE) {
    ...
    // Same reasoning as Chains immediately above — Pandora's Boxes is
    // also a no-timer, lock-in-gated reveal (see initPandorasBoxes's own
    // endsAt handling in this file and in ChallengeHost.jsx), so it
    // needs the identical safety net for a client that never got to
    // render the reveal and fire its own reportScore call.
    await autoLockResolvedScores({ gameId, db, round, playersById }, "pandorasboxes", pandorasBoxesKey, (s) => s.phase === "revealed", pandorasBoxesPlacementValue);
    // Musical Chairs: unlike Chains/Masquerade/Torched/Pandora's Boxes,
    // this one keeps a REAL challenge.endsAt (see ChallengeHost.jsx —
    // its round timings are sized off that same configured duration,
    // same relationship Scavenger Hunt's own per-round timer has to its
    // outer timer), so both halves of its safety net are needed: the
    // per-round phase clock (a player's browser tab drives this too,
    // via its own interval — this covers the gap when nobody's is) and
    // the challenge-level timeout fallback for the rare run of
    // unlucky-long rounds that outlasts it.
    await autoAdvanceMusicalChairs({ gameId, db, round });
    await autoFinalizeMusicalChairsOnTimeout({ gameId, db, round, playersById });
    await autoLockResolvedScores({ gameId, db, round, playersById }, "musicalchairs", musicalChairsKey, (s) => s.gamePhase === "revealed", musicalChairsPlacementValue);
    // The Floor: same reasoning as Musical Chairs immediately above —
    // a real challenge.endsAt sized off the same configured duration
    // (see ChallengeHost.jsx), so it needs both halves of the safety
    // net too: the per-action clock (choosing an opponent, or a duel
    // gone quiet — a player's own client also drives this, via its own
    // interval; this covers the gap when nobody's is) and the
    // challenge-level timeout fallback for a genuinely unlucky run of
    // stalls that outlasts it.
    await autoAdvanceFloor({ gameId, db, round });
    await autoFinalizeFloorOnTimeout({ gameId, db, round, playersById });
    await autoLockResolvedScores({ gameId, db, round, playersById }, "floor", floorKey, (s) => s.gamePhase === "revealed", floorPlacementValue);
    // Art Auction: same reasoning as Floor immediately above — a real
    // challenge.endsAt sized off the same configured duration (see
    // ChallengeHost.jsx and lib/games/artAuctionData.js's own
    // computePhaseDurationsMs), so it needs the same two-part safety
    // net: the per-phase clock (painting timing out into bidding, or
    // bidding timing out into settlement — a player's own client also
    // drives this, via its own interval; this covers the gap when
    // nobody's is) and the challenge-level timeout fallback for a
    // genuinely unlucky combination of a short battle and both phases'
    // own required minimums.
    await autoAdvanceArtAuction({ gameId, db, round });
    await autoFinalizeArtAuctionOnTimeout({ gameId, db, round, playersById });
    await autoLockResolvedScores({ gameId, db, round, playersById }, "artauction", artAuctionKey, (s) => s.phase === "revealed", artAuctionPlacementValue);
    // Mystery Button: same two-part safety net as Art Auction/Floor
    // immediately above — the per-pass clock (only ever matters for
    // Scenario B, once it's actually live; a no-op otherwise, same as
    // a player's own client polling — see
    // components/games/MysteryButtonPlayer.jsx) and the challenge-level
    // timeout fallback for whichever scenario never fully resolved on
    // its own.
    await autoAdvanceMysteryButton({ gameId, db, round });
    await autoFinalizeMysteryButtonOnTimeout({ gameId, db, round, playersById });
    await autoLockResolvedScores({ gameId, db, round, playersById }, "mysterybutton", mysteryButtonKey, (s) => s.phase === "revealed", mysteryButtonPlacementValue);

    // Split Friction: same reasoning as Art Auction/Mystery Button
    // immediately above — a real challenge.endsAt sized off the same
    // configured duration (see ChallengeHost.jsx and
    // lib/games/splitFrictionData.js's own computePhaseDurationsMs), so
    // it needs the same two-part safety net: the per-phase clock
    // (offering timing out into deciding, or deciding timing out into
    // settlement — a player's own client also drives this, via its own
    // interval; this covers the gap when nobody's is) and the
    // challenge-level timeout fallback for a genuinely unlucky
    // combination of a short battle and both phases' own required
    // minimums.
    await autoAdvanceSplitFriction({ gameId, db, round, settings });
    await autoFinalizeSplitFrictionOnTimeout({ gameId, db, round, playersById });
    await autoLockResolvedScores({ gameId, db, round, playersById }, "splitfriction", splitFrictionKey, (s) => s.phase === "revealed", splitFrictionPlacementValue);

    // Crowns / Crowns — Big Screen (see lib/games/crownsData.js's own
    // comment on tickCrowns) — trades are entirely player-initiated,
    // there's no phase clock of its own to drive, so this only ever
    // needs to catch one thing: the outer challenge timer running out
    // before anyone completes the full set. Both game types share the
    // exact same underlying state/mechanic (see initCrowns's own gate
    // above), so both are covered by the same tick + lock-in pair.
    await autoResolveCrowns({ gameId, db, round });
    await autoLockResolvedScores({ gameId, db, round, playersById }, "crowns", crownsKey, (s) => s.gameEnded, crownsPlacementValue);
    await autoLockResolvedScores({ gameId, db, round, playersById }, "crownstv", crownsKey, (s) => s.gameEnded, crownsPlacementValue);

    // Poseidon's Pool / Poseidon's Pool — Big Screen (see
    // lib/games/poseidonsPoolData.js's own comment on
    // tickPoseidonsPool) — shots are entirely player-initiated and
    // resolve via a queue the phone/TV polls already drain, so this
    // only ever needs to catch two things: a queued shot nobody's tab
    // happens to be open to drain, and the outer challenge timer
    // running out before the table's down to one ball. Both game types
    // share the exact same underlying state/mechanic (see
    // initPoseidonsPool's own gate below), so both are covered by the
    // same tick + lock-in pair.
    await autoResolvePoseidonsPool({ gameId, db, round });
    await autoLockResolvedScores({ gameId, db, round, playersById }, "poseidonspool", poseidonsPoolKey, (s) => s.gameEnded, poseidonsPoolPlacementValue);
    await autoLockResolvedScores({ gameId, db, round, playersById }, "poseidonspooltv", poseidonsPoolKey, (s) => s.gameEnded, poseidonsPoolPlacementValue);

    // The Golden Fleece — Big Screen (see lib/games/goldenFleeceData.js's
    // own comment on tickGoldenFleece) — same belt-and-suspenders
    // reasoning as every other shared timed battle here: the TV and
    // every active player's own phone already drive this on their own
    // poll while open, this just keeps a decision window resolving even
    // if neither happens to be open at the exact moment it times out.
    await autoResolveGoldenFleece({ gameId, db, round, settings });
    await autoLockResolvedScores({ gameId, db, round, playersById }, "goldenfleece", goldenFleeceKey, (s) => s.gameEnded, goldenFleecePlacementValue);

    // Gods and Gambits — Big Screen (see lib/games/godsAndGambitsData.js's
    // own comment on tickGodsAndGambits) — same belt-and-suspenders
    // reasoning as Golden Fleece immediately above: the TV and every
    // active player's own phone already drive this on their own poll
    // while open, this just keeps a guessing/betting decision window (or
    // the reveal pause) resolving even if neither happens to be open at
    // the exact moment it's due.
    await autoResolveGodsAndGambits({ gameId, db, round, settings });
    await autoLockResolvedScores({ gameId, db, round, playersById }, "godsandgambits", godsAndGambitsKey, (s) => s.gameEnded, godsAndGambitsPlacementValue);

    // The Diviner's Dice — Big Screen (see lib/games/divinersDiceData.js's
    // own comment on tickDivinersDice): same belt-and-suspenders
    // reasoning as Golden Fleece/Gods and Gambits above, but matters
    // MORE here — this game has no fixed round count of its own (see
    // that file's header), so this tick genuinely is what stands
    // between a stalled roll/decision window and the battle just
    // running out its outer challenge timer with nothing resolved.
    await autoResolveDivinersDice({ gameId, db, round, settings });
    await autoLockResolvedScores({ gameId, db, round, playersById }, "divinersdice", divinersDiceKey, (s) => s.gameEnded, divinersDicePlacementValue);

    // River Styx / The Wine-Dark Sea — Big Screen (see
    // lib/games/riverStyxData.js's own comment on tickRiverStyx): same
    // reasoning as Golden Fleece immediately above, just for a
    // turn-based idle-timeout instead of a decision-window timeout.
    await autoResolveRiverStyx({ gameId, db, round, settings });
    await autoLockResolvedScores({ gameId, db, round, playersById }, "riverstyx", riverStyxKey, (s) => s.gameEnded, riverStyxPlacementValue);
    await autoResolveWineDarkSea({ gameId, db, round, settings });
    await autoLockResolvedScores({ gameId, db, round, playersById }, "winedarksea", wineDarkSeaKey, (s) => s.gameEnded, wineDarkSeaPlacementValue);

    // Majority Rules — regular mode (see lib/games/majorityRulesData.js's
    // own comment on tickMajorityRules): same belt-and-suspenders
    // reasoning as Golden Fleece above — every player's own phone
    // already polls this on its own while open, this just keeps it
    // finalizing (and scores locked in) even if nobody has their phone
    // open at the exact moment every answer's in or the deadline hits.
    await autoResolveMajorityRules({ gameId, db, round, settings });
    await autoLockResolvedScores({ gameId, db, round, playersById }, "majorityrules", majorityRulesKey, (s) => s.finalized, (s, playerId) => s.results?.pointsByPlayer?.[playerId] ?? 0);

    // Majority Rules — Big Screen (see lib/games/majorityRulesTvData.js's
    // own comment on tickMajorityRulesTv): same reasoning, just for the
    // sequential TV mode's per-question phase clock instead of a single
    // all-at-once deadline.
    await autoResolveMajorityRulesTv({ gameId, db, round, settings });
    await autoLockResolvedScores({ gameId, db, round, playersById }, "majorityrulestv", majorityRulesTvKey, (s) => s.gameEnded, majorityRulesTvPlacementValue);
    ...
    // Torched's round resolution (see autoResolveTorchedRound's own
    // comment, and lib/games/torchedData.js's header for the full
    // simultaneous-rounds mechanic) — the overall battle now has a real
    // timer too (see the endsAt computation above), so this just needs
    // to keep rounds cycling along; the usual challenge-timeout
    // fallback (further down, once round.phase moves past CHALLENGE)
    // covers the case where the battle's own clock runs out before
    // anyone actually wins.
    await autoResolveTorchedRound({ gameId, db, round, settings });
    await autoLockResolvedScores({ gameId, db, round, playersById }, "torched", torchedKey, (s) => !!s.winnerId, torchedPlacementValue);

    // Simon — Big Screen (see lib/games/simonTvData.js's own comment
    // on resolveSimonRound) — same belt-and-suspenders reasoning as
    // Torched just above: the TV display already drives this on its
    // own 1s poll while it's open, this is just what keeps the round
    // resolving even if nobody has that page open at the moment a
    // round's own input window actually runs out.
    await resolveSimonRound(gameId, round.round, settings, db);
    await autoLockResolvedScores({ gameId, db, round, playersById }, "simontv", simonTvKey, (s) => !!s.winnerId, simonTvPlacementValue);

    // Musical Chairs — Big Screen (see lib/games/musicalChairsTvData.js's
    // own comment on resolveMusicalChairsRound) — same belt-and-
    // suspenders reasoning as Simon just above.
    await resolveMusicalChairsRound(gameId, round.round, settings, db);
    await autoLockResolvedScores({ gameId, db, round, playersById }, "musicalchairstv", musicalChairsTvKey, (s) => !!s.winnerId, musicalChairsTvPlacementValue);

    // Midas's Hoard — Big Screen (see lib/games/midasHoardData.js's own
    // header comment on midasHoardTransition) — same belt-and-suspenders
    // reasoning as Musical Chairs just above: the TV (and every phone)
    // already drive this on their own 500ms poll while open, this just
    // keeps rounds resolving if nobody happens to have either open at
    // the exact moment a phase's window runs out.
    await tickMidasHoard(gameId, round.round, db);
    await autoLockResolvedScores({ gameId, db, round, playersById }, "midashoard", midasHoardKey, (s) => !!s.winnerId, midasHoardPlacementValue);

    // Chariots of Conspire — Big Screen (see lib/games/chariotsData.js's
    // own header comment on chariotsTransition) — deliberately
    // continuous, like Wager Trivia/Acrophobia just above: there's no
    // winnerId or "revealed" state of its own, seats just keep
    // changing hands until the outer challenge timer ends the Battle
    // for everyone, at which point each phone's own reportScore(final:
    // true) call (see components/games/ChariotsPlayer.jsx) is the
    // entire scoring mechanism. So this only needs the tick, never
    // autoLockResolvedScores — there's nothing here to lock early.
    {
      const chariotsChallenge = await db.get(gameId, KEY_CHALLENGE);
      if (chariotsChallenge?.active && chariotsChallenge.gameType === "chariots") {
        await tickChariots(gameId, round.round, db);
      }
    }

    // Scylla's Strait — Big Screen (see lib/games/scyllasStraitData.js's
    // own header comment on scyllasStraitTransition) — a real bracket-
    // style winner/elimination game like Musical Chairs/Midas's Hoard
    // above, NOT continuous, so this pairs a plain gameType-guarded
    // tick (every phone and the TV already drive this on their own
    // poll while open, this just keeps a choosing/reveal window
    // resolving if neither happens to be open at the exact moment it's
    // due) with autoLockResolvedScores once winnerId is set.
    {
      const scyllaChallenge = await db.get(gameId, KEY_CHALLENGE);
      if (scyllaChallenge?.active && scyllaChallenge.gameType === "scyllasstrait") {
        await tickScyllasStrait(gameId, round.round, db);
      }
    }
    await autoLockResolvedScores({ gameId, db, round, playersById }, "scyllasstrait", scyllasStraitKey, (s) => !!s.winnerId, scyllasStraitPlacementValue);

    // Eyes in the System — Big Screen (see lib/games/
    // eyesInTheSystemTvData.js's own comment on resolveEyesTvMatch) —
    // same belt-and-suspenders reasoning as every other shared timed
    // battle here: the TV (and each face-off participant's own phone)
    // already drive this on their own poll while open, this just keeps
    // it moving if neither happens to be open at the exact moment a
    // match becomes resolvable.
    await resolveEyesTvMatch(gameId, round.round, db);
    await autoLockResolvedScores({ gameId, db, round, playersById }, "eyesinthesystemtv", eyesTvKey, (s) => !!s.winnerId, eyesTvPlacementValue);

    // Balloono (see lib/games/balloonoData.js's own comments on
    // resolveExplosions and tickBubbles) — same belt-and-suspenders
    // reasoning as every other shared timed battle here: the TV (and
    // each monkey's own phone) already drive both of these on their
    // own 300ms poll while open, this just keeps bombs detonating and
    // bubbles resolving if nobody happens to have either screen open
    // at the exact moment one becomes due.
    await resolveBalloonoExplosions(gameId, round.round, db);
    await tickBalloonoBubbles(gameId, round.round, db);
    await autoLockResolvedScores({ gameId, db, round, playersById }, "balloono", balloonoKey, (s) => !!s.winnerId, balloonoPlacementValue);

    // Wager Trivia — Big Screen (see lib/games/wagerTriviaTvData.js's
    // own header comment on wagerTriviaTransition) — same belt-and-
    // suspenders reasoning as every other shared timed battle here.
    // Continuous like Laurel Thief (no winnerId/lock-in — each
    // player's own client reports its running score via reportScore,
    // see components/games/WagerTriviaTvPlayer.jsx), so this only
    // needs to keep phases advancing, not lock anything in here.
    {
      const wagerChallenge = await db.get(gameId, KEY_CHALLENGE);
      if (wagerChallenge?.active && wagerChallenge.gameType === "wagertriviatv") {
        await tickWagerTrivia(gameId, round.round, db);
      }
    }

    // Trigger Happy — Big Screen (see lib/games/triggerHappyTvData.js's
    // own header comment on tickTriggerHappyTv) — same open-ended,
    // no-game-over-state shape as Wager Trivia immediately above: duels
    // just keep happening back-to-back until the outer challenge timer
    // ends the round for everyone, so this only ever needs the tick
    // (non-responder/reveal-pause/AFK-picker timeouts), never
    // autoLockResolvedScores — there's nothing here to lock early.
    {
      const triggerHappyChallenge = await db.get(gameId, KEY_CHALLENGE);
      if (triggerHappyChallenge?.active && triggerHappyChallenge.gameType === "triggerhappytv") {
        await tickTriggerHappyTv(gameId, round.round, settings, db);
      }
    }

    // Acrophobia — Big Screen (see lib/games/acrophobiaData.js's own
    // header comment on acrophobiaTransition) — same belt-and-suspenders
    // reasoning as every other shared timed battle here. Continuous
    // like Wager Trivia/Laurel Thief (no ended/winnerId to lock in —
    // each player's own client reports its running score via
    // reportScore, see components/games/AcrophobiaTvPlayer.jsx), so
    // this only needs to keep phases advancing, not lock anything in
    // here.
    {
      const acroChallenge = await db.get(gameId, KEY_CHALLENGE);
      if (acroChallenge?.active && acroChallenge.gameType === "acrophobia") {
        await tickAcrophobia(gameId, round.round, db);
      }
    }

    // Real-Time Mini Golf — Big Screen (see lib/games/miniGolfData.js's
    // own header comment on miniGolfTransition) — same belt-and-
    // suspenders reasoning as every other shared timed battle here: the
    // TV (and each player's own phone) already drive the physics on
    // their own 200ms poll while open, this just keeps balls rolling
    // and obstacles moving if neither happens to be open at the exact
    // moment a step comes due. Has a real ended/winnerIds like Tartarus
    // Treadmill (first three to finish, not continuous), so it locks in
    // once decided.
    await tickMiniGolf(gameId, round.round, db);
    await autoLockResolvedScores({ gameId, db, round, playersById }, "minigolf", miniGolfKey, (s) => !!s.ended, miniGolfPlacementValue);

    // Tartarus Treadmill — Big Screen (see lib/games/
    // tartarusTreadmillData.js's own header comment on
    // tartarusTreadmillTransition) — same belt-and-suspenders reasoning
    // as every other shared timed battle here: the TV (and each
    // player's own phone) already drive this on their own poll while
    // open, this just keeps the belt/obstacles advancing if neither
    // happens to be open at the exact moment a step comes due.
    await tickTartarusTreadmill(gameId, round.round, db);
    await autoLockResolvedScores({ gameId, db, round, playersById }, "tartarustreadmill", tartarusTreadmillKey, (s) => !!s.ended, tartarusTreadmillPlacementValue);

    // Spyfall (see lib/games/spyfallData.js's own header comment) —
    // played live/in-person, so there's no board to keep animating;
    // the only time-driven event is an accusation vote that's still
    // short of full consensus once its window closes. Same belt-and-
    // suspenders reasoning as everything else here: each player's own
    // phone already drives this on its own poll while open, this just
    // keeps a stalled vote from hanging forever if nobody's phone
    // happens to be open at the exact moment the window closes.
    await tickSpyfall(gameId, round.round, db);
    await autoLockResolvedScores({ gameId, db, round, playersById }, "spyfall", spyfallKey, (s) => !!s.ended, spyfallPlacementValue);
  ...
  if (round.phase === PHASES.CHALLENGE) {
    ...
    if (exile?.votingOpen) {
      // Dionysus's own character power (see lib/characterPowers.js)
      // makes them structurally unable to cast a vote at all — see
      // components/ExileVotePlayer.jsx's own Dionysus branch, which
      // never even shows them the picker. The missed-vote strike loop
      // near the bottom of this function already knows to skip them
      // for that exact reason ("confirmed against the actual voting UI
      // before assuming" — see its own comment); this check needs the
      // identical exemption for the identical reason, or a season with
      // Dionysus in play can never auto-close a vote at all — it was
      // requiring a ballot from someone who can never submit one, so
      // every Exile Vote fell all the way through to whatever the
      // configured timeout was (a real, reported symptom: a vote
      // configured for 16 hours sitting stuck rather than closing the
      // moment everyone who COULD vote had).
      const voters = players.filter((p) => p.approved && p.alive && powerFor(p, settings) !== "Dionysus");
  ...
  } else if (round.phase === PHASES.FATES) {
    // healNominatorOrder needs to run here, in housekeeping, and NOT
    // only from inside advanceFromFates further down — the gate right
    // below this whole if/else-if chain (isPhaseFullyDone + timerUp)
    // is exactly what was silently preventing it from ever being
    // reached at all. That gate exists to decide "should we even
    // attempt to advance the phase yet" — for Fates specifically, that
    // means "has every one of the top 3 nominated, or has the full
    // fatesDurationSec window elapsed" — and neither of those can ever
    // become true on its own if one of the three nominator slots is
    // occupied by someone who's no longer actually alive (see this
    // function's own comment on losingReentrantIds for how that
    // happens): they can never submit a nomination, so
    // isPhaseFullyDone stays false forever, and the gate falls all the
    // way through to waiting on the full multi-hour timeout every
    // single time — exactly the stuck-for-a-full-day symptom this was
    // actually reported as. Running the heal here, in housekeeping,
    // BEFORE that gate, means the nominator slot gets fixed
    // regardless of whether the gate would otherwise let anything
    // through — once it's fixed, the promoted player has a completely
    // normal chance to nominate, and the existing gate logic (further
    // down, and again inside advanceFromFates) starts working exactly
    // as it was always supposed to.
    const fates = await db.get(gameId, KEY_FATES);
    if (fates) {
      await healNominatorOrder({ gameId, db, client, postMessage, now, round, settings, players, alivePlayers, playersById }, fates);
    }
    ...
    if (finale?.phase === "voting" && finale.votingOpen) {
      // Same Dionysus exemption as the Exile Vote check just above, for
      // the identical reason — see components/FinalePlayer.jsx's own
      // Dionysus branch and the missed-finale-vote strike loop below,
      // which already carries this same skip.
      const voters = players.filter((p) => p.approved && !p.alive && powerFor(p, settings) !== "Dionysus");
  ...
  // Wrapped through logPhaseOutcome (see lib/debugLog.js) rather than
  // returned directly — this is what gives every phase-advance decision
  // an actual timestamped record instead of only living in whatever
  // the caller happened to do with the return value. Deliberately only
  // logs when something's actually noteworthy (an advance happened, OR
  // the reason isn't one of the routine "nothing to do yet" cases every
  // ordinary poll returns) — logging literally every ~10-second poll
  // regardless of outcome would drown the one bug worth finding in
  // thousands of harmless no-ops.
  if (round.phase === PHASES.CHALLENGE) return logPhaseOutcome(gameId, client, force, round, await advanceFromChallenge({ gameId, db, client, postMessage, now, round, settings, players, alivePlayers, playersById }), triggeredBy);
  if (round.phase === PHASES.FATES) return logPhaseOutcome(gameId, client, force, round, await advanceFromFates({ gameId, db, client, postMessage, now, round, settings, players, alivePlayers, playersById }), triggeredBy);
  if (round.phase === PHASES.EXILE) return logPhaseOutcome(gameId, client, force, round, await advanceFromExile({ gameId, db, client, postMessage, now, round, settings, players, alivePlayers, playersById }), triggeredBy);
  if (round.phase === PHASES.FINALE) return logPhaseOutcome(gameId, client, force, round, await advanceFromFinale({ gameId, db, client, postMessage, now, round, settings, players, alivePlayers, playersById }), triggeredBy);
...
async function recordExileHistoryIfMissing(env, exile) {
  ...
  const voteRows = filterCancelledVote(
    expandVoteRows(votes), // see lib/characterPowers.js's own comment on why Apollo's second vote is handled right here, as an extra row, rather than anywhere in the actual tallying logic
...
async function setPlayerAlive(client, gameId, playerId, alive, eliminationType, eliminationRound) {
  ...
  // See lib/seasonPlacement.js — coming back alive (re-entry) clears any
  // placement they'd have shown until, if ever, they're eliminated again.
  if (alive) await clearElimination(client, gameId, playerId);
  else await recordElimination(client, gameId, playerId);
}

// The Favor of the Fates "draw" — secretly picks which of the N mystery
...
async function autoStartRandomChallenge(env) {
  ...
  const disabledTypes = [...(settings.disabledChallenges || []), ...(await fetchGloballyDisabledChallenges()), ...presetIncompleteGameTypes(players), ...bigScreenOnlyGameTypesToExclude(settings)];

  let gameType;
  if (hephaestusPlayer) {
    const drawKey = hephaestusDrawKey(round.round);
    const drawRes = await db.update(gameId, drawKey, (fresh) => (fresh ? fresh : { options: pickHephaestusOptions(challengeHistory, disabledTypes, Date.now(), settings.challengeDurationSec, exileHistory), chosen: null, drawnAt: Date.now() }));
    if (!drawRes.ok) return;
    // Both options are "unlocked" for the Help tab's rules list the
    // moment they're offered to Hephaestus — whether or not either one
    // ends up chosen (see lib/unlockedGames.js). Idempotent, so calling
    // this again on every poll while the draw's still pending is fine.
    if (drawRes.value?.options) await markGameTypesUnlocked(gameId, drawRes.value.options, db);
    if (drawRes.value?.chosen) {
      gameType = drawRes.value.chosen;
    } else {
      // Hephaestus can pick any time from the moment this round's
      // challenge phase begins (see components/HephaestusChoice.jsx) —
      // but without SOME limit, a player who's away from the app would
      // silently stall the entire game forever: this function just
      // returns without ever starting the challenge as long as chosen
      // stays null, and nothing else in roundEngine.js has any other
      // path that starts one. Grace period reuses the season's own
      // configured challengeDurationSec (falling back to 900s/15min
      // for infinite-time seasons, same default this file already uses
      // for endsAt below) rather than a separate hardcoded value — a
      // season already running 16-hour rounds should give Hephaestus
      // real time to decide too, not the same fixed window a 5-minute
      // season would get.
      //
      // Measured from round.roundStartedAt, not drawnAt — deliberately.
      // Hephaestus can now pick the NEXT round's game while THIS round
      // is still in Battle/Fates/Exile (see the pre-draw right after
      // this round itself began, in advanceFromExile below), meaning
      // this round's own draw can genuinely have been created hours
      // before this round's own challenge phase actually starts.
      // drawnAt reflects when the draw was CREATED, which in that case
      // is the wrong clock to measure a grace period against — it would
      // start counting down while the PRECEDING round was still
      // running, and could already be expired the moment this round
      // even begins. roundStartedAt is fixed the instant THIS round
      // itself starts (see its own comment further down, on
      // KEY_ROUND) and never resets until the NEXT round does, which is
      // the actual clock this grace period needs.
      const graceMs = (settings.challengeDurationSec || 900) * 1000;
      if (Date.now() - (round.roundStartedAt || drawRes.value?.drawnAt || 0) < graceMs) return; // still within his window — keep waiting
      const autoPicked = Math.random() < 0.5 ? drawRes.value.options[0] : drawRes.value.options[1];
      const autoRes = await db.update(gameId, drawKey, (fresh) => (fresh?.chosen ? fresh : { ...fresh, chosen: autoPicked, autoChosen: true }));
      if (!autoRes.ok || !autoRes.value?.chosen) return;
      gameType = autoRes.value.chosen;
    }
  } else {
    const pickKey = randomPickKey(round.round);
    const pickRes = await db.update(gameId, pickKey, (fresh) => (fresh ? fresh : { gameType: pickRandomChallenge(challengeHistory, disabledTypes, Date.now(), settings.challengeDurationSec, exileHistory) }));
  ...
  const now = Date.now();
  // chains and pandorasboxes also removed from here — same reasoning
  // as torched/masquerade above, but these needed real new work first
  // (not just this flag): neither ever reported a score until the
  // whole thing resolved, so without adding real interim reporting
  // (see chainsData.js's and pandorasBoxesData.js's own placementValue
  // comments) a straggler timing out at the deadline would have
  // flattened EVERY participant's ranking to a flat no-show, including
  // players who'd already finished their entire side of the game.
  // settings.infiniteTime remains the one deliberate, host-controlled
  // way to still get an unbounded battle for any game type.
  ...
  // components/ChallengePlayer.jsx and lib/reentryData.js's own comment
  // on setReentryPreDecision) gets folded straight into it as a real
  // competitor, instead of needing to opt in all over again now that
  // it's live. Deliberately done as a SEPARATE update, only once this
  // specific call has already confirmed (via startToken above) that it
  // won the challenge-creation race — consuming pre-decisions any
  // earlier could clear an exiled player's opt-in on behalf of a
  // concurrent call that then lost that race and never actually created
  // anything.
  const preDecidedInIds = await consumePreDecidedReentryIns(gameId, db);
  if (preDecidedInIds.length > 0) {
    await db.update(gameId, KEY_CHALLENGE, (fresh) => {
      if (!fresh || fresh.startToken !== startToken) return fresh;
      const nextParticipantIds = [...new Set([...(fresh.participantIds || []), ...preDecidedInIds])];
      const nextDecisions = { ...(fresh.reentryDecisions || {}) };
      preDecidedInIds.forEach((id) => { nextDecisions[id] = "in"; });
      const nextAttemptIds = [...new Set([...(fresh.reentryAttemptIds || []), ...preDecidedInIds])];
      return { ...fresh, participantIds: nextParticipantIds, reentryDecisions: nextDecisions, reentryAttemptIds: nextAttemptIds };
    });
  }

  // See lib/unlockedGames.js — a straight random pick's own gameType is
  // "unlocked" for the Help tab's rules list the moment its battle
  // actually starts (Hephaestus's own two options are already marked
  // unlocked earlier, at draw time, above).
  await markGameTypesUnlocked(gameId, gameType, db);

  // Battle-start notification for players — genuinely missing before
  // this: notifyRoundChange already covers "a new round has begun" and
  // "the Fates Ceremony/Exile Vote has opened," but nothing previously
  // fired the moment a challenge itself actually became playable,
  // which for a randomly-selected game (this function) can be a real
  // amount of time after the round itself began — Hephaestus's own
  // pick window, or the grace period before an auto-pick, both delay
  // this past "Round N Begins." Uses the same notifyRoundChange
  // helper, not a separate one, so it inherits the exact same
  // web+native push behavior and the player's own notify_rounds
  // preference without duplicating that logic.
  const startedLabel = GAME_REGISTRY[gameType]?.label || gameType;
  await notifyRoundChange(gameId, `⚔️ ${startedLabel} Has Started`, "The Battle is live — go compete!");
  ...
  if (gameType === "wordscrambletv") await initWordScrambleTv(gameId, round.round, participants, now, db);
  if (gameType === "simontv") await initSimonTv(gameId, round.round, participants, now, db);
  if (gameType === "musicalchairstv") await initMusicalChairsTv(gameId, round.round, participants, now, db);
  if (gameType === "eyesinthesystem") await initEyesInTheSystem(gameId, round.round, participants, now, db);
  if (gameType === "eyesinthesystemtv") await initEyesInTheSystemTv(gameId, round.round, participants, now, db);
  if (gameType === "balloono") await initBalloono(gameId, round.round, participants, now, db);
  if (gameType === "laurelthief" || gameType === "laurelthieftv") await initLaurelThief(gameId, round.round, participants, now, db);
  if (gameType === "wagertriviatv") await initWagerTrivia(gameId, round.round, participants, now, db);
  if (gameType === "tartarustreadmill") await initTartarusTreadmill(gameId, round.round, participants, now, db);
  if (gameType === "spyfall") await initSpyfall(gameId, round.round, participants, now, db);
  if (gameType === "acrophobia") await initAcrophobia(gameId, round.round, participants, now, db);
  if (gameType === "minigolf") await initMiniGolf(gameId, round.round, participants, now, db);
  ...
  if (gameType === "pandorasboxes") await initPandorasBoxes(gameId, round.round, participants, db);
  if (gameType === "musicalchairs") await initMusicalChairs(gameId, round.round, participants, now, settings.challengeDurationSec, db);
  if (gameType === "floor") await initFloor(gameId, round.round, participants, now, settings.challengeDurationSec, db);
  if (gameType === "artauction") await initArtAuction(gameId, round.round, participants, now, settings.challengeDurationSec, db);
  if (gameType === "mysterybutton") await initMysteryButton(gameId, round.round, participants, now, settings.challengeDurationSec, db);
  if (gameType === "scavengerhunt") await initScavengerHunt(gameId, round.round, participants, now, db);
  if (gameType === "splitfriction") await initSplitFriction(gameId, round.round, participants, now, settings.challengeDurationSec, db);
  if (gameType === "crowns" || gameType === "crownstv") await initCrowns(gameId, round.round, participants, now, db);
  if (gameType === "poseidonspool" || gameType === "poseidonspooltv") await initPoseidonsPool(gameId, round.round, participants, now, settings.challengeDurationSec, db);
  if (gameType === "midashoard") await initMidasHoard(gameId, round.round, participants, now, db);
  if (gameType === "chariots") await initChariots(gameId, round.round, participants, now, db);
  if (gameType === "scyllasstrait") await initScyllasStrait(gameId, round.round, participants, now, db);
...
// ─── Musical Chairs: per-round phase clock ───
// A thin server-side mirror of the exact same call every player's own
// client already makes on its own interval (see
// components/games/MusicalChairsPlayer.jsx) — advanceMusicalChairsIfDue
// is a no-op unless the fresh state's own timestamps say a transition
// is actually due, so calling it redundantly from both places is
// exactly as safe as it is everywhere else in this file. This is what
// keeps a round moving even in the (real, if rare) window where every
// player's tab is backgrounded at once — a locked phone screen between
// "the music just stopped" and "I pick up my phone."
async function autoAdvanceMusicalChairs(env) {
  const { gameId, db, round } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "musicalchairs") return;
  await advanceMusicalChairsIfDue(gameId, round.round, db);
}

// ─── Musical Chairs: challenge-level timeout fallback ───
// Same two-mechanism shape as autoAdvanceScavengerRound's own file
// header comment describes for Scavenger Hunt: the per-round clock
// above is the normal path, this is the outer bound for when an
// unlucky run of long, randomized music phases (see
// lib/games/musicalChairsData.js's own comment on why the total isn't
// guaranteed to fit the configured duration) burns through the whole
// challenge before the bracket ever reduces itself to one winner.
// Without this, nobody still standing would ever see a reveal (their
// reportScore never fires) and nobody eliminated earlier would have
// their proper points either.
async function autoFinalizeMusicalChairsOnTimeout(env) {
  const { gameId, db, round } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "musicalchairs" || !challenge.endsAt) return;
  if (Date.now() < challenge.endsAt) return;

  await db.update(gameId, musicalChairsKey(round.round), (fresh) => finalizeMusicalChairsOnTimeout(fresh));
}

// ─── The Floor: per-action clock ───
// A thin server-side mirror of the exact same calls every player's own
// client already makes on its own interval (see
// components/games/FloorPlayer.jsx) — both are no-ops unless the fresh
// state's own timestamps say a transition is actually due, so calling
// them redundantly from both places is exactly as safe as it is
// everywhere else in this file. Covers the same gap Musical Chairs'
// equivalent above does: every player's tab backgrounded at once,
// between "it's your turn to pick" (or "the duel needs a next
// question") and someone actually looking at their phone again.
async function autoAdvanceFloor(env) {
  const { gameId, db, round } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "floor") return;
  await autoChooseFloorOpponentIfDue(gameId, round.round);
  await autoResolveFloorDuelIfDue(gameId, round.round);
}

// ─── The Floor: challenge-level timeout fallback ───
// Same two-mechanism shape as Musical Chairs' own pair immediately
// above: the per-action clock is the normal path, this is the outer
// bound for when a genuinely unlucky run of stalls (or just a very
// large bracket) burns through the whole configured challenge duration
// before one player ever controls the entire floor. Without this,
// nobody still standing would ever see a reveal (their reportScore
// never fires) and nobody already eliminated would have their proper
// points locked in either.
async function autoFinalizeFloorOnTimeout(env) {
  const { gameId, db, round } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "floor" || !challenge.endsAt) return;
  if (Date.now() < challenge.endsAt) return;

  await db.update(gameId, floorKey(round.round), (fresh) => finalizeFloorOnTimeout(fresh));
}

// ─── Art Auction: per-phase clock ───
// A thin server-side mirror of the exact same calls every player's own
// client already makes on its own interval (see
// components/games/ArtAuctionPlayer.jsx) — both are no-ops unless the
// fresh state's own timestamps say a transition is actually due, so
// calling them redundantly from both places is exactly as safe as it
// is everywhere else in this file. Covers the same gap Floor and
// Musical Chairs' equivalents above do: every player's tab backgrounded
// at once, between "painting time's up" (or "bidding's closed") and
// someone actually looking at their phone again.
async function autoAdvanceArtAuction(env) {
  const { gameId, db, round } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "artauction") return;
  await autoOpenBiddingIfDue(gameId, round.round);
  await autoResolveAuctionIfDue(gameId, round.round);
}

// ─── Art Auction: challenge-level timeout fallback ───
// Same two-mechanism shape as Floor and Musical Chairs' own pairs
// above: the per-phase clock is the normal path, this is the outer
// bound for when a genuinely short battle collides with both phases'
// own required minimums (see lib/games/artAuctionData.js's
// computePhaseDurationsMs) and runs past the season's configured
// challenge duration before the auction ever closes naturally. Without
// this, nobody would ever see a reveal (their reportScore never
// fires) and the whole battle would just hang.
async function autoFinalizeArtAuctionOnTimeout(env) {
  const { gameId, db, round } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "artauction" || !challenge.endsAt) return;
  if (Date.now() < challenge.endsAt) return;

  await db.update(gameId, artAuctionKey(round.round), (fresh) => finalizeArtAuctionOnTimeout(fresh));
}

// ─── Split Friction: phase clock ───
// A thin server-side mirror of the exact same call every player's own
// client already makes on its own interval (see
// components/games/SplitFrictionPlayer.jsx) — tickSplitFriction already
// re-checks everything itself (not resolved yet, either everyone's
// locked in or the current phase's window has timed out), so calling it
// redundantly from both places is exactly as safe as it is everywhere
// else in this file. No TV for this game (it's phone-only), so unlike
// Golden Fleece's three-way redundancy this is just the two-way
// phone-poll + server-housekeeping pattern goldenFleeceData.js's own
// header describes for the no-TV case.
async function autoAdvanceSplitFriction(env) {
  const { gameId, db, round, settings } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "splitfriction") return;
  await tickSplitFriction(gameId, round.round, settings, db);
}

// ─── Split Friction: challenge-level timeout fallback ───
// Same two-mechanism shape as Art Auction's own pair above: the
// per-phase clock is the normal path, this is the outer bound for when
// a genuinely short battle collides with both phases' own required
// minimums (see lib/games/splitFrictionData.js's computePhaseDurationsMs)
// and runs past the season's configured challenge duration before the
// battle ever settles naturally. Without this, nobody would ever see a
// reveal (their reportScore never fires) and the whole battle would
// just hang.
async function autoFinalizeSplitFrictionOnTimeout(env) {
  const { gameId, db, round } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "splitfriction" || !challenge.endsAt) return;
  if (Date.now() < challenge.endsAt) return;

  await db.update(gameId, splitFrictionKey(round.round), (fresh) => finalizeSplitFrictionOnTimeout(fresh));
}

// ─── Mystery Button: per-pass clock ───
// A thin server-side mirror of the exact same call every player's own
// client already makes on its own interval (see
// components/games/MysteryButtonPlayer.jsx) — a no-op unless the fresh
// state's own passDeadline says a forced pass is actually due (and
// only relevant at all once Scenario B is live with a real holder —
// see autoPassIfDue's own guards), so calling it redundantly from both
// places is exactly as safe as it is everywhere else in this file.
// Covers the same gap every other per-action clock in this file does:
// every player's tab backgrounded at once between "you've got it, pass
// it on" and someone actually looking at their phone again.
async function autoAdvanceMysteryButton(env) {
  const { gameId, db, round } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "mysterybutton") return;
  await autoPassMysteryButtonIfDue(gameId, round.round);
}

// ─── Mystery Button: challenge-level timeout fallback ───
// Same two-mechanism shape as every other multi-phase battle's pair
// above: the per-pass clock is the normal path, this is the outer
// bound for when Scenario A never got 3 presses, or Scenario B's own
// per-pass safety net still wasn't enough to finish the whole chain
// before the season's configured challenge duration ran out. Without
// this, nobody would ever see a reveal (their reportScore never
// fires) and the whole battle would just hang.
async function autoFinalizeMysteryButtonOnTimeout(env) {
  const { gameId, db, round } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "mysterybutton" || !challenge.endsAt) return;
  if (Date.now() < challenge.endsAt) return;

  await db.update(gameId, mysteryButtonKey(round.round), (fresh) => finalizeMysteryButtonOnTimeout(fresh));
}
...
// Torched's simultaneous-round resolution — same defensive
// re-verification pattern as autoTimeoutMasquerade just above (check
// against fresh data inside the update callback, not just the outer
// read), but the actual mechanic is entirely different now: Torched
// switched from turn-based to simultaneous timed rounds (see
// lib/games/torchedData.js's own header comment for the full
// reasoning — this was a deliberate, requested redesign, not a
// timeout tweak). All the actual round-readiness logic (everyone
// submitted, or the round's own timer ran out) now lives in
// resolveRound itself, so this is just the poll that calls it.
async function autoResolveTorchedRound(env) {
  const { gameId, db, round, settings } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "torched") return;

  const state = await db.get(gameId, torchedKey(round.round));
  if (!state || !state.shooting || state.winnerId) return; // not shooting yet, or already decided — resolveRound itself re-checks this too, this is just to skip an unnecessary write attempt

  await resolveTorchedRound(gameId, round.round, settings, db);
}

// ─── The Golden Fleece: decision-window resolution ───
// Same shape as autoResolveTorchedRound just above — tickGoldenFleece
// already re-checks everything itself (not resolved yet, either
// everyone's chosen or the window's timed out), this is purely to skip
// an unnecessary write attempt when the challenge isn't even this game.
async function autoResolveGoldenFleece(env) {
  const { gameId, db, round, settings } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "goldenfleece") return;
  await tickGoldenFleece(gameId, round.round, settings, db);
}

// ─── Gods and Gambits: guessing/betting decision-window + reveal-pause
// resolution ───
// Same shape as autoResolveGoldenFleece just above — tickGodsAndGambits
// already re-checks everything itself (not resolved yet, either
// everyone's responded or the current phase's window has timed out),
// this is purely to skip an unnecessary write attempt when the active
// challenge isn't even this game.
// ─── Crowns / Crowns — Big Screen: outer-timer safety net ───
// Same shape as autoResolveGoldenFleece just above — tickCrowns already
// re-checks everything itself (not resolved yet, and the outer
// challenge.endsAt has genuinely passed), this is purely to skip an
// unnecessary write attempt when the active challenge isn't even one of
// these two game types, and to hand tickCrowns the one thing it needs
// that it has no other way to get: the battle's own real
// challenge.endsAt (this game deliberately has no phase timer of its
// own — see lib/games/crownsData.js's header).
async function autoResolveCrowns(env) {
  const { gameId, db, round } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || (challenge.gameType !== "crowns" && challenge.gameType !== "crownstv")) return;
  await tickCrowns(gameId, round.round, challenge.endsAt, db);
}

// ─── Poseidon's Pool / Poseidon's Pool — Big Screen: shot-queue drain +
// outer-timer safety net ───
// Same shape as autoResolveCrowns just above — tickPoseidonsPool
// already re-checks everything itself (not resolved yet, not still
// mid-replay of the last shot), this is purely to skip an unnecessary
// write attempt when the active challenge isn't one of these two game
// types, and to hand it the one thing it needs that it has no other way
// to get: the battle's own real challenge.endsAt.
async function autoResolvePoseidonsPool(env) {
  const { gameId, db, round } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || (challenge.gameType !== "poseidonspool" && challenge.gameType !== "poseidonspooltv")) return;
  await tickPoseidonsPool(gameId, round.round, challenge.endsAt, db);
}

async function autoResolveGodsAndGambits(env) {
  const { gameId, db, round, settings } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "godsandgambits") return;
  await tickGodsAndGambits(gameId, round.round, settings, db);
}

async function autoResolveDivinersDice(env) {
  const { gameId, db, round, settings } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "divinersdice") return;
  await tickDivinersDice(gameId, round.round, settings, db);
}

// ─── River Styx / The Wine-Dark Sea: turn-timeout stall safety net ───
// Same shape as autoResolveGoldenFleece just above — tickRiverStyx/
// tickWineDarkSea already re-check everything themselves (a turn only
// ever gets auto-played once it's genuinely timed out), this is purely
// to skip an unnecessary write attempt when the challenge isn't even
// one of these two games.
async function autoResolveRiverStyx(env) {
  const { gameId, db, round, settings } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "riverstyx") return;
  await tickRiverStyx(gameId, round.round, settings, db);
}
async function autoResolveWineDarkSea(env) {
  const { gameId, db, round, settings } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "winedarksea") return;
  await tickWineDarkSea(gameId, round.round, settings, db);
}

// ─── Majority Rules (regular mode + Big Screen): belt-and-suspenders tick ───
// Same shape as autoResolveGoldenFleece above — tickMajorityRules/
// tickMajorityRulesTv already re-check everything themselves (only ever
// finalizing once every participant's locked in, or the battle's own
// endsAt/per-question window has genuinely timed out), this is purely
// to skip an unnecessary write attempt when the active challenge isn't
// even this game.
async function autoResolveMajorityRules(env) {
  const { gameId, db, round, settings } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "majorityrules") return;
  await tickMajorityRules(gameId, round.round, settings, db);
}
async function autoResolveMajorityRulesTv(env) {
  const { gameId, db, round, settings } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "majorityrulestv") return;
  await tickMajorityRulesTv(gameId, round.round, settings, db);
}
...
async function advanceFromChallenge(env) {
  ...
  // Same bug this function's own comment just above already fixed for
  // Final Four's alive-count check, but that fix never reached this
  // line — a losing re-entry attempt (anyone in reentryAttemptIds who
  // ISN'T the winner) was still being passed straight through into
  // computeChallengeOutcome's own top-3 slice for a normal round,
  // meaning a failed re-entrant who happened to place 2nd or 3rd in
  // the underlying battle score still got treated as a real top-3
  // finisher and asked to nominate at the Fates Ceremony — despite
  // being permanently eliminated the moment their re-entry attempt
  // lost. Filtering them out HERE, before the slice, rather than
  // trying to remove them from outcome.nominators afterward, is what
  // actually matters: a post-hoc removal would just leave the
  // ceremony one nominator short instead of correctly pulling in
  // whoever placed next (4th, 5th, ...) among the players who are
  // actually still in the game.
  const losingReentrantIds = reentryAttemptIds.filter((id) => id !== winnerId);
  const outcomePlacements = losingReentrantIds.length > 0
    ? challenge.placements.filter((p) => !losingReentrantIds.includes(p.playerId))
    : challenge.placements;
  const outcome = computeChallengeOutcome(outcomePlacements, isFinalFour);
  ...
  if (!challenge.finalized) {
    ...
    if (reentryAttemptIds.length > 0) {
      ...
      for (const attemptId of reentryAttemptIds) {
        ...
        if (updatedEntry.status === REENTRY_STATUS.RETURNED) {
          await setPlayerAlive(client, gameId, updatedEntry.playerId, true, null, null);
    ...
    // Majority Rules' own per-question reveal (see
    // lib/games/majorityRulesData.js's header comment) — nobody ever
    // sees the majority or anyone else's picks while the battle's live,
    // so the full breakdown (who was pitted against whom, what the room
    // decided, and how each player answered) only ever surfaces here, on
    // the standard history entry every finishing battle already gets.
    // Attached directly rather than through some generic "extra payload"
    // mechanism — this is the only game type that needs one.
    let majorityRulesReveal = null;
    if (challenge.gameType === "majorityrules") {
      const mrState = await db.get(gameId, majorityRulesKey(round.round));
      if (mrState?.results) {
        majorityRulesReveal = {
          questions: mrState.questions.map((q, i) => ({
            id: q.id,
            text: q.text,
            playerAId: q.playerAId,
            playerBId: q.playerBId,
            playerAName: mrState.participantNames?.[q.playerAId] || "?",
            playerBName: mrState.participantNames?.[q.playerBId] || "?",
            majoritySide: mrState.results.perQuestion[i]?.majoritySide ?? null,
            tallyA: mrState.results.perQuestion[i]?.tallyA ?? 0,
            tallyB: mrState.results.perQuestion[i]?.tallyB ?? 0,
          })),
          participantIds: mrState.participantIds,
          participantNames: mrState.participantNames,
          answers: mrState.answers,
    ...
    const historyRes = await db.update(gameId, KEY_CHALLENGE_HISTORY, (fresh) => {
      ...
      return [...list, {
        round: round.round, placements: challenge.placements, winnerId: outcome.winner?.playerId || null, finalFour: isFinalFour, gameType: challenge.gameType || "manual",
        ...(majorityRulesReveal ? { majorityRulesReveal } : {}),
      }];
    });
...
async function advanceFromFates(env) {
  ...
  // Self-healing check first — see healNominatorOrder's own comment
  // for why this needs to run before preference resolution and the
  // timeout fallback below, not after: a nominator slot that gets
  // fixed here should give whoever's promoted into it a completely
  // normal chance to nominate, not get immediately auto-picked too.
  fates = await healNominatorOrder(env, fates);
  ...
  if (!roundRes.ok) throw new Error("Couldn't move the round to the Exile Vote — try again.");
  await postMessage(
    `In Olympus, all crave power. Power, however, comes at a cost. Whom are you willing to betray, to manipulate, to exile?\n\n⚖️ Fates Ceremony complete!\n\nNominees for exile: ${nominees.map((n) => n.name).join(", ")}.\n\n🃏 The Favor of the Fates is up for grabs this round — everyone gets one shot to claim it.\n\n➡️ Moving to the Exile Vote${round.doubleElimination ? " — this is a DOUBLE ELIMINATION round, so votes are cast to SAVE, not eliminate." : ""}.`
  );
  await notifyRoundChange(gameId, "🗳️ Exile Vote", `Nominees: ${nominees.map((n) => n.name).join(", ")}. Time to vote.`);
  return { advanced: true, to: PHASES.EXILE };
}

// ─── No-holder tie-break fallback ───
// Normally a tie waits on the Favor of the Fates holder's own pick (the
// `exile.chaosHolderId`-gated branches inline in advanceFromExile,
// below). But chaosHolderId is only ever set by someone actually
// winning that round's mystery-card draw (see pages/api/chaos-draw.js)
// — it starts null every round and STAYS null whenever every eligible
// voter's draw pick misses. That's a real, regularly-reachable outcome
// (see ExileVoteHost.jsx's "Voting's closed with nobody claiming it —
// no one holds it this round" message), not just a theoretical edge
// case. Without a holder, nothing can ever write `exile.tieBreakChoiceId`,
// so the normal "wait for the holder" path would just hang forever on a
// tie with no one able to resolve it. This falls back to whichever tied
// nominee placed worst in that same round's Battle (see
// resolveEliminateTieByChallenge in lib/exileLogic.js); if that's STILL
// tied — or nobody in the tie has any recorded placement at all (didn't
// play that Battle) — it breaks the remaining tie at random.
// Resolves synchronously in a single pass (no external actor needs to
// act), and is idempotent/guarded the same way the holder-wait message
// below is, so a fast re-poll doesn't double-post or re-roll the pick.
async function resolveNoHolderTieBreak(env, exile, tiedIds, roundNum) {
  const { gameId, db, postMessage } = env;
  if (exile.tieBreakChoiceId) return exile.tieBreakChoiceId; // already resolved by an earlier pass

  const challenge = await db.get(gameId, KEY_CHALLENGE);
  const placements = challenge && challenge.round === roundNum ? challenge.placements || [] : [];
  const byChallenge = resolveEliminateTieByChallenge(tiedIds, placements);
  const byId = {};
  (exile.nominees || []).forEach((n) => (byId[n.playerId] = n.name));

  let resolvedId;
  let announcement;
  if (Array.isArray(byChallenge)) {
    resolvedId = pickRandomAmong(byChallenge);
    const noData = byChallenge.length === tiedIds.length;
    announcement = `🃏 No Favor of the Fates holder this round — breaking the tie by lowest challenge score...\n\n${noData ? "No challenge results on record for the tied nominees" : "Still tied on challenge score"} — eliminating at random: ${byId[resolvedId] || "?"}.`;
  } else {
    resolvedId = byChallenge;
    announcement = `🃏 No Favor of the Fates holder this round — breaking the tie by lowest challenge score: ${byId[resolvedId] || "?"} placed worst in the Battle.`;
  }

  if (!exile.noHolderTieBreakAnnounced) {
    await postMessage(announcement);
    await db.update(gameId, KEY_EXILE, (fresh) => (fresh ? { ...fresh, noHolderTieBreakAnnounced: true } : fresh));
  }
  await db.update(gameId, KEY_EXILE, (fresh) => (fresh && !fresh.tieBreakChoiceId ? { ...fresh, tieBreakChoiceId: resolvedId } : fresh));
  return resolvedId;
}
...
async function advanceFromExile(env) {
  ...
  // Defensive re-check, not just relying on the caller (advancePhase's
  // own `!force` block, above) having already confirmed voting is
  // genuinely closed before ever calling this function. That gate is
  // SUPPOSED to make this unreachable until either every alive,
  // approved, voting-eligible player has voted or the configured
  // voteDurationSec window has actually elapsed — a real season hit
  // exactly this symptom once: a Round 1 Exile Vote configured for 16
  // hours concluded within minutes, with only a handful of votes ever
  // recorded, and several players who simply hadn't voted yet got
  // wrongly struck for "missing" a vote that was still supposed to be
  // open. That turned out to have a real, findable cause after all
  // (see the matching fix in the housekeeping check above, in
  // advancePhase): the eligible-voters computation wasn't excluding
  // Dionysus, whose own character power makes them structurally unable
  // to vote at all (see components/ExileVotePlayer.jsx) — meaning
  // "everyone eligible has voted" could never become true in ANY
  // season with Dionysus in play, no matter how fast everyone else
  // voted, since it was waiting on a ballot from someone who can never
  // submit one. This defensive re-check stays regardless, now correctly
  // carrying the same exemption — a second, independent guard against
  // this exact class of bug recurring is worth keeping even with the
  // root cause identified.
  const eligibleVoters = (alivePlayers || []).filter((p) => p.approved && powerFor(p, settings) !== "Dionysus");
  const everyoneVoted = eligibleVoters.length > 0 && eligibleVoters.every((p) => !!votes[p.id]);
  const windowElapsed = settings.infiniteTime ? false : (round.phaseStartedAt != null && now - round.phaseStartedAt >= (settings.voteDurationSec || 0) * 1000);
  // Not given its own dedicated log call here — the outer wrapper
  // (logPhaseOutcome, in advancePhase) already logs any blocked reason
  // that isn't in its own routine-exclusion list, and
  // "exile-vote-window-still-open" deliberately isn't in that list, so
  // every time this early return actually fires, it's already
  // captured with a real timestamp automatically. That's exactly the
  // anomaly worth a permanent record of: this function is only
  // supposed to be reachable once the caller's own gate has already
  // confirmed voting is genuinely closed, so this line firing at all
  // means that didn't hold.
  if (!everyoneVoted && !windowElapsed) {
    return { advanced: false, reason: "exile-vote-window-still-open" };
  }
  ...
  // The Favor of the Fates gets exercised no matter what, picked at random
  ...
  if (exile.chaosHolderId && !nullifiedId) {
    ...
    if (!timedOut) {
      ...
      if (!exile.holderWaitAnnounced) {
        await postMessage(`🃏 Waiting on the Favor of the Fates holder's decision before the vote can be finalized.`);
    ...
    const forcedReason = "(Auto-selected — the Favor of the Fates holder made no decision in time.)";
    ...
    if (forceError) { console.error("Failed to force-exercise the Favor of the Fates:", forceError); return { advanced: false, reason: "waiting-on-holder-decision" }; }
    ...
    const holderName = playersById[exile.chaosHolderId] || "The Favor of the Fates holder";
    let khaosAnnouncement = `🃏 ${holderName} made no Favor of the Fates decision within ${formatDurationHours(settings.voteDurationSec)} — the game has exercised it on their behalf.\n\nAs a consequence, they'll be barred from competing in next round's Battle.`;
    if (strikeResult.applied) khaosAnnouncement += ` They also received an inactivity strike.`;
    if (strikeResult.removed) {
      khaosAnnouncement += `\n\n🚫 They reached 3 inactivity strikes and have been removed from the game.`;
      await postSystemChatMessage(env, `🚫 ${holderName} reached 3 inactivity strikes and has been removed from the game.`);
    }
  ...
  if (exile.mode === "save") {
    ...
    if (outcome.needsTieBreak && !exile.tieBreakChoiceId) {
      if (!exile.chaosHolderId) {
        // No Favor of the Fates holder exists this round to ever break
        // this — fall back to challenge score / random instead of
        // waiting forever. See resolveNoHolderTieBreak, above.
        exile.tieBreakChoiceId = await resolveNoHolderTieBreak(env, exile, outcome.tied, round.round);
      } else {
        ...
        if (!exile.tieBreakAnnounced) {
          await postMessage(`🃏 The vote to save is tied. Waiting on the Favor of the Fates holder to break the tie.`);
    ...
    if (outcome.needsTieBreak && !exile.tieBreakChoiceId) {
      if (!exile.chaosHolderId) {
        // No Favor of the Fates holder exists this round — see
        // resolveNoHolderTieBreak, above.
        exile.tieBreakChoiceId = await resolveNoHolderTieBreak(env, exile, outcome.tied, round.round);
      } else {
        // Same guard as above, same reasoning.
        if (!exile.tieBreakAnnounced) {
          await postMessage(`🃏 The vote is tied. Waiting on the Favor of the Fates holder to break the tie.`);
          await db.update(gameId, KEY_EXILE, (fresh) => (fresh ? { ...fresh, tieBreakAnnounced: true } : fresh));
        }
        return { advanced: false, reason: "waiting-on-tiebreak" };
      }
  ...
  if (!exile.revealed) {
    ...
    if (missedVoteNames.length > 0 || removedForVoteNames.length > 0) {
      ...
      if (removedForVoteNames.length > 0) {
        msg += `${msg ? "\n\n" : ""}🚫 ${removedForVoteNames.join(", ")} reached 3 inactivity strikes and have been removed from the game.`;
        await postSystemChatMessage(env, `🚫 ${removedForVoteNames.join(", ")} reached 3 inactivity strikes and ${removedForVoteNames.length > 1 ? "have" : "has"} been removed from the game.`);
      }
      await postMessage(msg);
    }

    for (const id of exiledIds) {
      await setPlayerAlive(client, gameId, id, false, "exiled", round.round);
      const name = exile.nominees.find((n) => n.playerId === id)?.name || "?";
      await db.update(gameId, KEY_REENTRY, (fresh) => {
        const list = fresh || [];
        const idx = list.findIndex((r) => r.playerId === id);
        if (idx === -1) return [...list, { playerId: id, name, status: REENTRY_STATUS.PENDING, exiledRound: round.round }];
        const existing = list[idx];
        // Same round already has an entry — this is a duplicate
        // processing attempt of the SAME exile (a retry, a re-run of
        // this housekeeping pass), not a genuinely new one. Preserve
        // it untouched, same as the original behavior here.
        if (existing.exiledRound === round.round) return list;
        // A different, earlier round's entry still sitting here means
        // this player already went through the whole exile ->
        // re-entry arc once before and is only reachable by this loop
        // again because they actually made it back (RETURNED) and got
        // exiled a second time — see lib/reentryLogic.js's own "every
        // exiled player gets exactly ONE re-entry attempt" rule, which
        // was never meant to mean "one attempt for their entire time
        // in the game" — a player who successfully returned earned a
        // completely fresh shot at it this time, not permanent
        // ineligibility carried over from an entry that predates them
        // even being back in the game. ELIMINATED_FOREVER shouldn't
        // actually be reachable here at all (that status means
        // they're already permanently out, never alive to be
        // nominated/exiled again) — but resetting it to PENDING too,
        // rather than leaving it as a dead-end case with no explicit
        // handling, is the honest, consistent choice if it's ever
        // somehow hit.
        const updated = [...list];
        updated[idx] = { ...existing, status: REENTRY_STATUS.PENDING, exiledRound: round.round };
        return updated;
      });
  ...
  if (remainingAlive.length <= 1) {
    ...
    // Best-effort — a failure here shouldn't ever block the game
    // itself from actually ending, which is why this isn't awaited
    // into the same try/throw flow as the writes above it.
    evaluateAndAwardAchievements(gameId, client, db).catch(() => {});
  ...
  if (remainingAlive.length <= 3) {
    ...
    if (!existingFinale) {
      // Jury-eligible only (see lib/finaleQaData.js's isJuryEligible) —
      // a player who quit or was removed for inactivity never gets a
      // jury voice, and that includes a shot at the Favor of the Fates
      // draw. This has to stay in lockstep with pages/api/chaos-draw.js's
      // own finale poolSize count and ChaosPowerPlayer.jsx's button
      // count: the winning index picked here is only ever valid if it's
      // less than however many mystery buttons actually get offered.
      const exiledPool = players.filter(isJuryEligible);
    ...
    await postMessage(
      `🔥 We've reached the Finale! Finalists: ${remainingAlive.map((p) => p.display_name).join(", ")}.\n\nVoting is open now — every exiled player who didn't quit or wasn't removed votes for a winner, and the Favor of the Fates is up for grabs among them too. Finalists can write their statement and answer jury questions the whole time voting is open.`
    );
  ...
  // Pre-draws Hephaestus's two options for the round AFTER this one
  // (nextRound + 1), the moment THIS round (nextRound) begins — not
  // waiting for nextRound's own Battle/Fates/Exile to finish first.
  // This is what actually lets him choose "at any point during the
  // preceding round" the way his power is meant to work: from this
  // exact moment, through nextRound's entire Battle, Fates Ceremony,
  // and Exile Vote, the draw already exists and his choice (if he
  // makes one) is already locked in by the time nextRound + 1 itself
  // begins — autoStartRandomChallenge's own draw-creation (using
  // `fresh ? fresh : {...}`) already reuses whatever's here rather
  // than overwriting it, so this needs no other change there beyond
  // the grace-period fix (see that function's own comment on why it's
  // now measured from roundStartedAt, not drawnAt — this is exactly
  // the situation that made the old version wrong: a draw created a
  // full round early, sometimes hours before it's actually needed).
  if (settings.challengeSelectionMode === "random") {
    const hephaestusPlayer = remainingAlive.find((p) => p.approved && powerFor(p, settings) === "Hephaestus");
    if (hephaestusPlayer) {
      const challengeHistory = (await db.get(gameId, KEY_CHALLENGE_HISTORY)) || [];
      const exileHistory = (await db.get(gameId, KEY_EXILE_HISTORY)) || [];
      const disabledTypes = [...(settings.disabledChallenges || []), ...(await fetchGloballyDisabledChallenges()), ...presetIncompleteGameTypes(remainingAlive), ...bigScreenOnlyGameTypesToExclude(settings)];
      const preDrawRes = await db.update(gameId, hephaestusDrawKey(nextRound + 1), (fresh) => (fresh ? fresh : {
        options: pickHephaestusOptions(challengeHistory, disabledTypes, Date.now(), settings.challengeDurationSec, exileHistory),
        chosen: null, drawnAt: Date.now(),
      }));
      // See lib/unlockedGames.js — offered to Hephaestus is "unlocked"
      // for the Help tab's rules list, same as the other draw-creation
      // site in autoStartRandomChallenge above.
      if (preDrawRes.value?.options) await markGameTypesUnlocked(gameId, preDrawRes.value.options, db);
    }
  }
...
async function advanceFromFinale(env) {
  ...
  if (finale.chaosHolderId && !nullifiedId) {
    ...
    if (!timedOut) {
      if (!finale.holderWaitAnnounced) {
        await postMessage(`🃏 Waiting on the Favor of the Fates holder's decision before the finale vote can be finalized.`);
    ...
    const holderName = playersById[finale.chaosHolderId] || "The Favor of the Fates holder";
    let khaosAnnouncement = `🃏 ${holderName} made no Favor of the Fates decision within ${formatDurationHours(settings.voteDurationSec)} — the game has exercised it on their behalf.`;
  ...
  if (outcome.needsTieBreak && !finale.tieBreakChoiceId) {
    ...
    if (!finale.tieBreakAnnounced) {
      await postMessage(`🃏 The finale vote is tied. Waiting on the exiled Favor of the Fates holder to break the tie.`);
  ...
  if (missedFinaleVoteNames.length > 0 || removedForFinaleVoteNames.length > 0) {
    ...
    if (removedForFinaleVoteNames.length > 0) {
      inactivityMsg += `${inactivityMsg ? "\n\n" : ""}🚫 ${removedForFinaleVoteNames.join(", ")} reached 3 inactivity strikes.`;
      // In practice a juror is already not alive by this point (see
      // applyInactivityStrike's own "only actually removes a still-
      // alive player" comment), so this branch rarely fires — kept
      // consistent with every other removal point in this file anyway,
      // in case that ever changes.
      await postSystemChatMessage(env, `🚫 ${removedForFinaleVoteNames.join(", ")} reached 3 inactivity strikes and ${removedForFinaleVoteNames.length > 1 ? "have" : "has"} been removed from the game.`);
    }
  ...
  await postMessage(
    `🏆🔥 ${winner?.name || "No one"} wins Panopticon!` +
    (runnerUpName ? `\n🥈 ${runnerUpName} finishes 2nd.` : "") +
    (thirdName ? `\n🥉 ${thirdName} finishes 3rd${nullifiedId === third?.playerId ? " — nullified by the Favor of the Fates" : ""}.` : "")
  );
  // Best-effort — see advanceFromExile's identical comment on why this
  // is fire-and-forget rather than blocking the game's own ending.
  evaluateAndAwardAchievements(gameId, client, db).catch(() => {});
  return { advanced: true, to: PHASES.ENDED };
}
