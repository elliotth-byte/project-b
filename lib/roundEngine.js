...
import { nominationsComplete, distinctNominees, takenNomineeIds, autoPickNominee, formatDurationHours } from "./fatesLogic";
...
// If the battle winner (always nominatorOrder[0] — see lib/challengeLogic.js's
// `nominators: ranked.slice(0, 3)`) hasn't submitted their own Fates
...
// target on their behalf and bars them from the NEXT round's Battle as
// the stated consequence. If the season has infiniteTime on, there's no
// configured duration to match, so this never triggers at all — that's
// a deliberate reading of "match the configurable duration," not an
// oversight: no configured limit means no timeout to miss. Only ever
// touches the winner's own nomination slot — 2nd/3rd place nominators
// aren't subject to this rule. A no-op if the winner's already
// nominated, if the window hasn't closed yet, or (defensively) if
// there's genuinely no eligible player left to auto-pick.
async function autoNominateForWinnerIfTimedOut(env, fates) {
  const { gameId, db, client, round, settings, alivePlayers, playersById, now } = env;
  const winnerId = fates.nominatorOrder?.[0]?.playerId;
  if (!winnerId || fates.nominations?.[winnerId]) return fates; // already nominated — nothing to do
  ...
  const taken = takenNomineeIds(fates.nominations, winnerId);
  const autoPick = autoPickNominee(winnerId, winnerId, aliveIds, taken); // winnerId passed twice deliberately — same person is both "nominator" and "immune" here
  if (!autoPick) return fates; // no eligible target somehow — leave it for a human to sort out rather than guess

  const durationLabel = formatDurationHours(settings.fatesDurationSec);

  const fatesRes = await db.update(gameId, KEY_FATES, (fresh) => {
    if (!fresh || fresh.nominations?.[winnerId]) return fresh; // a concurrent call already handled this
    return {
      ...fresh,
      nominations: { ...fresh.nominations, [winnerId]: autoPick },
      nominationReasons: { ...(fresh.nominationReasons || {}), [winnerId]: `(Auto-selected — no nomination made within ${durationLabel}.)` },
    };
  });
  if (!fatesRes.ok) return fates;

  // Bars them from next round's Battle. round.round + 1 is safe to use
  // as "the next round's number" regardless of how many non-Battle
  // phases (Exile, etc.) happen before that Battle actually starts —
  // the round counter only increments at a brand new round beginning,
  // never at an intermediate phase change.
  const { error: banError } = await client.from("players").update({ battle_ban_round: round.round + 1 }).eq("id", winnerId);
  if (banError) console.error("Failed to set battle_ban_round:", banError);

  await env.postMessage(
    `⏳ ${playersById[winnerId] || "The winner"} didn't submit a Fates nomination within ${durationLabel} — the game has chosen ${playersById[autoPick] || "someone"} on their behalf.\n\nAs a consequence, ${playersById[winnerId] || "they"} will be barred from competing in next round's Battle.`
  );
...
async function advanceFromFates(env) {
  ...
  fates = await autoNominateForWinnerIfTimedOut(env, fates);
