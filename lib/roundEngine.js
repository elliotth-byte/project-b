...
export async function advancePhase(gameId, ctx) {
  ...
  // advanceFromExile's normal flow AND unconditionally right here on
  // every poll — so a stale, revealed-but-unrecorded exile left over
  // from an EARLIER round (exactly what a game already past that round
  // would have) gets backfilled the moment this runs, not just for a
  ...
  if (staleExile?.revealed && staleExile.round < round.round) {
    await recordExileHistoryIfMissing({ gameId, db, client }, staleExile);
  }
