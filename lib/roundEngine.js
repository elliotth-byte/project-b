...
async function advanceFromExile(env) {
  ...
  // approved player has voted or the configured voteDurationSec window
  // has actually elapsed — but a real season hit exactly this
  // symptom anyway: a Round 1 Exile Vote configured for 16 hours
  // concluded within minutes, with only a handful of the season's 14
  // players' votes ever recorded, and several of the players who
  // simply hadn't gotten to vote YET were wrongly given an inactivity
  // strike for "missing" a vote that was still, by every setting,
  // supposed to be open. Rather than leave this function trusting an
  // upstream gate that evidently isn't holding under some real
  // condition this couldn't be reproduced or pinned down by reading the
  // code alone, this re-derives the SAME "is voting actually over"
  // answer independently, right here, using nothing but this
  // function's own now/round/settings/players — the same three
  // ingredients the caller's gate itself is built from — and refuses
  // to touch strikes or compute an outcome unless it can verify that
  // for itself. Whatever the exact upstream path turns out to be, nothing
  // downstream of this point can now run early.
  const eligibleVoters = (alivePlayers || []).filter((p) => p.approved);
