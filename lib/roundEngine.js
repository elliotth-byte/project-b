...
import { applyZeusPower, computeAresImmunityUpdates, overrideStatusForHades, filterCancelledVote, powerFor } from "./characterPowers";
...
import { initMasquerade } from "./games/masqueradeData";
import { initCloseToTwenty } from "./games/closeToTwentyData";
import { initTorched } from "./games/torchedData";
import { initChains } from "./games/chainsData";
...
async function autoNominateTimedOutNominators(env, fates) {
  ...
  await env.postMessage(
    `⏳ ${names} didn't submit a Fates nomination within ${durationLabel} — the game has chosen on their behalf.\n\nAs a consequence, they'll be barred from competing in next round's Battle.`
  );
...
async function autoStartRandomChallenge(env) {
  ...
  if (hephaestusPlayer) {
    ...
    const drawRes = await db.update(gameId, drawKey, (fresh) => (fresh ? fresh : { options: pickHephaestusOptions(challengeHistory), chosen: null }));
    ...
    const pickRes = await db.update(gameId, pickKey, (fresh) => (fresh ? fresh : { gameType: pickRandomChallenge(challengeHistory) }));
...
async function advanceFromExile(env) {
  ...
  // use the power at all. Only once the window has actually elapsed
  // does a still-undecided holder get treated that way for real. If the
  // season has infiniteTime on, there's no configured window to wait
  // out at all, so the decision is awaited indefinitely — same as every
  // other phase respects infiniteTime by never auto-timing-out.
...
async function advanceFromFinale(env) {
  ...
  // Same gap as advanceFromExile above, same fix — a holder who hadn't
  // acted yet was being treated as though they'd declined to use the
  // power at all, the instant the last juror's vote came in. The Finale
  // has no duration/timer concept of its own at all (it already waits
  // indefinitely for every vote by design, unlike the Exile Vote which
  // has a configured window), so there's no timeout to apply here
  // either — the holder's decision is simply awaited for as long as it
  // takes, consistent with how this whole phase already behaves.
