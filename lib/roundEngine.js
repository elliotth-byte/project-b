...
export async function advancePhase(gameId, ctx) {
  ...
  if (settings.autoAdvance) {
    if (round.phase === PHASES.CHALLENGE) {
      // Same idea, for re-entry: an exiled player's opt-in/opt-out window
      // for THIS challenge stays open until every alive competitor has
      // actually finished — that's the deadline, not the challenge's
      // timer. Once that happens, anyone still undecided defaults to
      // "out" (costs them nothing — see lib/reentryLogic.js). Digital
      // challenges only: manual ones never add an undecided reentrant to
      // participantIds in the first place (the host just enters results
      // for whoever actually opted in), so there's nothing to default.
  ...
  if (!force) {
    ...
    if (round.phase === PHASES.EXILE || round.phase === PHASES.FINALE) {
      ...
      if (!timerUp) {
        // Even if the timer's still running (or there IS no timer —
        // infinite time), don't make everyone wait once they've all
        // actually finished their part: every participant done with the
        // challenge, or all three Fates nominations in. See
