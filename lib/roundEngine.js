...
export async function advancePhase(gameId, ctx) {
  ...
  // lib/reentryLogic.js). This always runs, independent of
  // settings.autoAdvance below — leaving someone's decision stuck in
  // limbo isn't something that toggle is meant to protect. Digital
  // challenges only: manual ones never add an undecided reentrant to
  // participantIds in the first place (the host just enters results for
  // whoever actually opted in), so there's nothing to default.
  if (round.phase === PHASES.CHALLENGE) {
    ...
    if (challenge?.active && !challenge.finalized && challenge.gameType && challenge.gameType !== "manual") {
      const undecided = (challenge.reentryEligibleIds || []).filter((id) => !challenge.reentryDecisions?.[id]);
      if (undecided.length > 0) {
        const eligibleSet = new Set(challenge.reentryEligibleIds);
    ...
    // automatically — saves the host a click. This IS gated by
    // autoAdvance (unlike re-entry above) since it's part of the same
    // "protect my reveal ceremony" toggle as the check below: it doesn't
    // touch anything the ceremony depends on (that's all local UI state
    // on the host's own screen — see isPhaseFullyDone's comment), but a
    // host who wants full manual control over Exile/Finale probably wants
    // it off here too.
  ...
  if (!force) {
    if (round.phase === PHASES.EXILE || round.phase === PHASES.FINALE) {
      // settings.autoAdvance is the host's off-switch for auto-advancing
      // these two (protects a live reveal ceremony) — a host running one
      // may want it off so the round doesn't jump forward mid-ceremony;
      // everything still works via the "Finish Now" button either way.
      if (!settings.autoAdvance) return { advanced: false, reason: "auto-advance-disabled" };
      // Never tally and finalize a vote that's still technically open —
      // the timer running out on its own is NOT enough here, unlike
      // Challenge/Fates below. advanceFromExile/advanceFromFinale don't
      // (and shouldn't) care how many votes are actually in; they just
      // tally whatever exists the moment they're called, which — if the
      // timer alone could trigger this — could mean finalizing on
      // whichever single vote happened to land first. Voting only
      // becomes eligible to close via the housekeeping above (everyone's
      // actually voted) or the host's own "Close Voting" / "Finish Now".
  ...
  if (!exile.revealed) {
    ...
    const historyRes = await db.update(gameId, KEY_EXILE_HISTORY, (fresh) => {
      ...
      return [...list, {
        ...
        chaosHolderId: exile.chaosHolderId, nullifiedId, nullifiedReason, tieBreakChoiceId: exile.tieBreakChoiceId || null,
        voteRows,
...
async function advanceFromFinale(env) {
  ...
  const finaleRes = await db.update(gameId, KEY_FINALE, (fresh) => ({ ...(fresh || finale), nullifiedFinalistId: nullifiedId, nullifiedReason, voteRows, winnerId, revealed: true }));
