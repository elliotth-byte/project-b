...
async function advanceFromExile(env) {
  ...
  if (!exile.revealed) {
    for (const id of exiledIds) {
      ...
      // adds this player. Best-effort: chat being off, or the migration
      // not being run, shouldn't ever block an actual exile from going
      // through, so a failure here is logged, not thrown.
