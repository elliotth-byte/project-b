...
import { initTorched, startShootingPhase, placementValue as torchedPlacementValue, torchedKey, nextAliveTurnIndex as nextAliveTorchedTurnIndex } from "./games/torchedData";
...
export async function advancePhase(gameId, ctx) {
  ...
  if (round.phase === PHASES.CHALLENGE) {
    ...
    // Torched's per-turn timeout (see autoTimeoutTorched's own comment)
    // — the overall battle now has a real timer too (see the endsAt
    // computation above), so this just needs to keep the turn cycling
    // along; the usual challenge-timeout fallback (further down, once
    // round.phase moves past CHALLENGE) covers the case where the
    // battle's own clock runs out before anyone actually wins.
    await autoTimeoutTorched({ gameId, db, round, settings });
...
// Torched's per-turn timeout — same shape and reasoning as
// autoTimeoutMasquerade just above, with one real difference in the
// consequence: Masquerade eliminates a stalled player outright, but
// Torched just passes the turn along. A player who's slow to respond
// in Masquerade is actively blocking someone else's poison choice from
// resolving; a player who's slow to call a coordinate in Torched isn't
// blocking anything for anyone else except the pace of the game — their
// own marker stays exactly where it was, still fully in play, and they
// get their next turn normally. Elimination here only ever happens the
// way the game's own rules say it should: getting hit.
//
// TARGET_TURNS_ESTIMATE=32 is what turns settings.challengeDurationSec
// into an actual per-turn window — chosen so a 16-hour battle (a real
// configured season length in this app, see the 16-Hour Round preset
// in components/AdminHost.jsx) lands on roughly 30 minutes per turn,
// which is the concrete example this was built from. Unlike
// Masquerade's own maxTurns (a real, provable upper bound on how many
// turns ITS structure can take), Torched has no equivalent hard bound
// — a long enough streak of misses could in principle run past however
// many turns 32 implies — so this is a deliberate target/estimate, not
// a guarantee, the same way lib/games/scavengerHuntData.js's own
// TARGET_ROUNDS is a floor on what's POSSIBLE rather than a promise
// about what actually happens. Clamped the same way that file's own
// round timeout is, for the same reason: too short and there's no real
// time to look at the board and think; too long and one turn could
// drag for hours even in an otherwise-fast season.
const TORCHED_TARGET_TURNS_ESTIMATE = 32;
const TORCHED_MIN_TURN_SEC = 30;
const TORCHED_MAX_TURN_SEC = 1800;

async function autoTimeoutTorched(env) {
  const { gameId, db, round, settings } = env;
  const challenge = await db.get(gameId, KEY_CHALLENGE);
  if (!challenge?.active || challenge.gameType !== "torched") return;

  const state = await db.get(gameId, torchedKey(round.round));
  if (!state || !state.turnOrder || state.winnerId) return; // not shooting yet, or already decided

  const totalSec = settings.challengeDurationSec || 900;
  const perTurnSec = Math.max(TORCHED_MIN_TURN_SEC, Math.min(TORCHED_MAX_TURN_SEC, totalSec / TORCHED_TARGET_TURNS_ESTIMATE));
  const perTurnMs = perTurnSec * 1000;

  if (!state.currentTurnStartedAt || Date.now() - state.currentTurnStartedAt < perTurnMs) return; // still within the window

  await db.update(gameId, torchedKey(round.round), (fresh) => {
    if (!fresh || !fresh.turnOrder || fresh.winnerId) return fresh;
    if (!fresh.currentTurnStartedAt || Date.now() - fresh.currentTurnStartedAt < perTurnMs) return fresh; // re-verify against FRESH data — a real shot may have landed in between
    return {
      ...fresh,
      currentTurnIndex: nextAliveTorchedTurnIndex(fresh, fresh.currentTurnIndex),
