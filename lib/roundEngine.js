...
async function healNominatorOrder(env, fates) {
  ...
  try {
    ...
    // Logged unconditionally (even the empty/nothing-to-do case) rather
    // than only on an actual swap — this is the one line that answers
    // "is this check even running at all" from the Debug Log tab,
    // which matters a lot more once something looks stuck than a log
    // line that only ever appears when it already worked.
    await logGameEvent(client, gameId, {
      event: "fates_heal_check", round: round?.round, phase: "fates", source: "healNominatorOrder",
      detail: summarize({ nominatorOrder: fates.nominatorOrder.map((n) => n.name), nominatedSoFar: Object.keys(fates.nominations || {}), aliveCount: aliveIds.size, needsReplacement: needsReplacement.map((n) => n.name) }),
    });
