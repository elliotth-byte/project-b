...
async function autoStartRandomChallenge(env) {
  ...
  if (hephaestusPlayer) {
    ...
    const drawRes = await db.update(gameId, drawKey, (fresh) => (fresh ? fresh : { options: pickHephaestusOptions(challengeHistory, disabledTypes, Date.now(), settings.challengeDurationSec, exileHistory), chosen: null }));
