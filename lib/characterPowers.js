...
export const CHARACTER_POWERS = [
  ...
  {
    name: "Ares", powerName: "Power of Rivalry", icon: "⚔️", phase: "At the beginning of the game", implemented: true,
  ...
  {
    name: "Hephaestus", powerName: "Power of the Forge", icon: "🔥", phase: "At the beginning of the challenge", implemented: true,
    description: "You can see two options for the next round's challenge and pick between the two of them.",
  },
  {
    name: "Hades", powerName: "Power of Afterlife", icon: "💀", phase: "At the beginning of the challenge", implemented: true,
  ...
  {
    name: "Athena", powerName: "Power of Manipulation", icon: "🦉", phase: "At the Favor of the Fates selection", implemented: true,
  ...
  {
    name: "Hermes", powerName: "Power of Insight", icon: "🪽", phase: "At the Favor of the Fates selection", implemented: true,
  ...
  {
    name: "Apollo", powerName: "Power of Amplification", icon: "🏹", phase: "During voting deliberation", implemented: true,
  ...
  {
    name: "Artemis", powerName: "Power of the Snare", icon: "🏕", phase: "During voting deliberation", implemented: true,
  ...
  {
    name: "Dionysus", powerName: "Power of Intoxication", icon: "🍇", phase: "During voting deliberation", implemented: true,
...
// ─── Apollo ───
// "You may cast a second vote at each elimination. You may cast your
// votes for the same player or two different players." Scoped to
// Exile Votes only ("at each elimination" — the Finale is a winner
// selection, not an elimination, so this deliberately doesn't touch
// lib/roundEngine.js's jury-vote path the way it touches both of the
// Exile Vote's own voteRows construction sites).
//
// The underlying votes object still only ever has ONE entry per voter
// (see components/ExileVotePlayer.jsx's submitVote) — Apollo's second
// choice lives as an extra secondTargetId field on that SAME entry,
// not a second top-level key, so every existing "has this player
// voted yet" check (`!!votes[p.id]`, used throughout roundEngine.js's
// auto-close and missed-vote-strike logic) keeps working completely
// unchanged: casting either or both of his votes in one submission
// still just means votes[apolloId] exists. This is what turns that one
// entry into two separate ROWS right before tallying — after this, the
// existing tally() in lib/exileLogic.js counts them as two ordinary
// votes with no idea either came from the same person, which is
// exactly the point: the counting rules themselves never needed to
// change, only how many rows a single vote entry produces.
export function expandVoteRows(votes) {
  const rows = [];
  Object.entries(votes || {}).forEach(([voterId, v]) => {
    rows.push({ voterId, targetId: v.targetId, reason: v.reason || null });
    if (v.secondTargetId) rows.push({ voterId, targetId: v.secondTargetId, reason: v.reason || null });
  });
  return rows;
}
