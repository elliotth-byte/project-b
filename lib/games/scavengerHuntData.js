...
// available.
//
// MAX_ROUND_SEC was originally 90 (a season-length preset topping out
// well under 20 minutes total, at the time this was written, made that
// the effective ceiling in every real case) — but this app has since
// grown genuinely long presets (a 16-hour round length, among them),
// and 90 seconds per round in a 16-hour Battle is exactly the "rounds
// were going way too quick" bug this was raised to fix: nobody checking
// in once an hour could ever act before their round moved on without
// them. Raised to 1800 (30 minutes) — long enough that an all-day
// Battle still gives real, humane time per round (roughly 32 rounds
// across 16 hours instead of a flat 90-second cadence regardless of
// how long the Battle actually runs), while still keeping a single
// round from dragging on for genuinely unbounded stretches if a host
// ever configures something even longer than that.
