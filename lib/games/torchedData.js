...
//
// A per-turn timeout DOES apply now, scaled to the season's own
// configured challengeDurationSec (see lib/roundEngine.js's
// autoTimeoutTorched, and its own comment on why the per-turn window
// is derived the same way Masquerade's already was) — a player who
// doesn't call a coordinate within their window simply has their turn
// passed to the next living player, with no other penalty; they're
// still on the board and still get their next turn normally. This
// replaces an earlier version of this comment that documented the
// opposite as an intentional tradeoff ("targeting waits indefinitely,
// never decided on their behalf") — that was fine when the overall
// battle itself also had no time limit, but stopped making sense once
// the battle got a real duration too (see the endsAt computation in
// lib/roundEngine.js): an unbounded per-turn wait inside a now-bounded
// overall battle just meant one inactive player could burn the whole
// battle's clock doing nothing.
//
// Placement itself still has no per-player deadline of its own beyond
// the overall battle timer — a player who never places simply isn't
// part of the turn order once it's set (see startShootingPhase), and
// is ranked at the bottom alongside anyone else who didn't
// participate. The per-turn timeout above only ever applies once
// shooting has actually started.
//
...
// initPlinkoBracket for the full reasoning (this needed the exact same
// fix once the round engine started calling it server-side too — see
// roundEngine.js's own autoTransitionTorchedPlacement, right before
...
export async function startShootingPhase(gameId, round, seed, db) {
  ...
  return update(gameId, key(round), (fresh) => {
    ...
    return { ...fresh, turnOrder: order, currentTurnIndex: 0, currentTurnStartedAt: Date.now() };
  });
}

// Exported so roundEngine.js's per-turn timeout check (autoTimeoutTorched)
// can reuse the exact same "who's next" logic when force-skipping a
// stalled player's turn, rather than duplicating it.
export function nextAliveTurnIndex(torched, fromIndex) {
...
export async function fireShot(gameId, round, playerId, row, col) {
  return storageUpdate(gameId, key(round), (fresh) => {
    ...
    return {
      ...
      // Reset the instant the turn actually changes hands — this is
      // what autoTimeoutTorched (see lib/roundEngine.js) measures a
      // per-turn window against. Left untouched (not reset to now) once
      // there's a winner, since nobody's turn is "starting" at that
      // point.
      currentTurnStartedAt: winnerId ? fresh.currentTurnStartedAt : Date.now(),
    };
  });
}
