...
export const REVEAL_DISPLAY_MS = 4000;

export const masqueradeKey = (round) => `pb:masquerade:${round}`;
const key = masqueradeKey;
...
export async function initMasquerade(gameId, round, participants, seed, db) {
  ...
  await set(gameId, key(round), {
    ...
    turn: {
      ...
      offerIsPoison: null,
      comment: null,
      // Character-turn timeout (see roundEngine.js's own
      // autoTimeoutMasquerade for the full reasoning) — deliberately set
      // ONCE per turn, here and in advanceAfterReveal's own next-turn
      // setup, and NEVER reset when submitTargetChoice moves targeting
      // into responding. One shared clock for the whole turn, not two
      // separate ones — see autoTimeoutMasquerade's comment for why that
      // distinction actually matters for the guarantee this exists to
      // provide.
...
//
// db: optional override — see lib/games/plinkoBracketData.js's
// initPlinkoBracket for why (needed here specifically so a stalling
// player getting auto-eliminated server-side — see roundEngine.js's
// autoTimeoutMasquerade — doesn't then ALSO need a client to click
// through the reveal before the game can keep moving; that would
// defeat the entire point).
export async function advanceAfterReveal(gameId, round, db) {
