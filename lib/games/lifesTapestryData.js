...
// a grid, and — like a real over/under weave — each square only ever
// belongs to ONE of the two threads crossing it. A cell at (r, c)
// where (r + c) is even is part of that row's horizontal thread and
// only moves when you shift ITS row; a cell where (r + c) is odd is
// part of that column's vertical thread and only moves when you shift
// ITS column. So a row's shift button doesn't rotate the whole row —
// it rotates just the alternating checkerboard squares that belong to
// that thread, skipping over the squares that are actually woven
// into a column instead (and vice versa for a column shift). This
// checkerboard split flips with r/c the same way a real weave's
// over/under pattern flips from row to row and column to column —
// fixed by user request after the original whole-row/whole-column
// version (every cell in a row moving together) proved too hard.
//
// Same "shuffle via simulated legal moves" solvability trick as
// lib/games/slidingPuzzleData.js: a checkerboard-subset shift is its
// own kind of move, and reversing that exact same move (opposite
// direction) always undoes it — so any sequence of these moves
// applied to the solved grid is, by construction, always fully
// solvable by moves of the same kind. No separate parity check is
// needed the way a raw random permutation of tiles would require —
// and note a cell can NEVER cross between belonging to a row and
// belonging to a column (that's the whole point: it's woven into
// exactly one thread), so solving is really ROWS + COLS small,
// fully-independent cyclic puzzles rather than one large tangled one.
//
...
// The checkerboard squares of row r that actually belong to ITS
// thread — (r + c) even — in ascending column order. Depending on r's
// own parity this is 3 cells or 2 cells wide (COLS=5 is odd, so the
// split isn't perfectly even, same as any real weave's edge threads).
function rowGroupPositions(r) {
  const cols = [];
  for (let c = 0; c < COLS; c++) if ((r + c) % 2 === 0) cols.push(c);
  return cols.map((c) => r * COLS + c);
}
// The checkerboard squares of column c that belong to ITS thread —
// (r + c) odd — in ascending row order. Always exactly 2 cells here
// since ROWS=4 is even, so every column splits evenly.
function colGroupPositions(c) {
  const rows = [];
  for (let r = 0; r < ROWS; r++) if ((r + c) % 2 === 1) rows.push(r);
  return rows.map((r) => r * COLS + c);
}
...
function rotatePositions(grid, positions, dir) {
  ...
  // other way — the first value wraps around to the back. A 2-cell
  // group is just a swap either way, which is expected, not a bug —
  // a real 2-thread crossing only has one other position to go to.
...
// dir: -1 = left, 1 = right — only rotates row r's own checkerboard
// thread (see rowGroupPositions above), leaving that row's other
// squares (woven into their columns) untouched.
export function shiftRow(grid, r, dir) {
  return rotatePositions(grid, rowGroupPositions(r), dir);
}
// dir: -1 = up, 1 = down — only rotates column c's own checkerboard
// thread (see colGroupPositions above), leaving that column's other
// squares (woven into their rows) untouched.
export function shiftCol(grid, c, dir) {
  return rotatePositions(grid, colGroupPositions(c), dir);
}
