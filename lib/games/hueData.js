...
// Takes an already-created `rand` generator rather than a seed
// directly, so generateTargetColors below can pull several targets in
// a row off the SAME seeded stream (see that function's own comment).
function pickOneTarget(rand) {
...
export function generateTargetColor(seed) {
  return pickOneTarget(seededRandom(seed));
}

// NUM_TARGETS targets in a row, sharing ONE seeded stream — same
// "identical for every player" fairness generateHourglassDurations
// already relies on elsewhere, just pulling several values off the
// stream instead of one. A single target could be matched and locked
// in within a few seconds, which ended the whole challenge for
// everyone almost as soon as it started; three in sequence (still the
// exact same per-color mixing/locking-in flow, just repeated) gives it
// real duration without changing what playing it actually feels like.
export const NUM_TARGETS = 3;

export function generateTargetColors(seed, count = NUM_TARGETS) {
  const rand = seededRandom(seed);
  return Array.from({ length: count }, () => pickOneTarget(rand));
}
...
// The multi-target round score: `targets`/`mixes` are same-length
// arrays (one mix per target, in order) — closeness is averaged across
// all of them (matching a color perfectly and whiffing another still
// nets a fair middle score, rather than one bad match tanking
// everything or being ignored). The speed bonus is deliberately
// computed ONCE, off the total time to lock in every color, not
// per-color — three separate small speed bonuses would reward rushing
// each individual color rather than the round as a whole, and would
// also need three separate timestamps threaded through instead of the
// one usePersistedStart value every other Battle already uses.
export function computeHueRoundScore(targets, mixes, elapsedMs, totalDurationMs) {
  const perColor = targets.map((target, i) => {
    const mix = mixes[i];
    const distance = Math.hypot(target.r - mix.r, target.g - mix.g, target.b - mix.b);
    return Math.max(0, 100 - (distance / MAX_RGB_DISTANCE) * 100);
  });
  const closeness = perColor.reduce((sum, c) => sum + c, 0) / perColor.length;
  const safeDuration = totalDurationMs > 0 ? totalDurationMs : 300000;
  const usedFraction = Math.max(0, Math.min(1, elapsedMs / safeDuration));
  const speedBonus = MAX_SPEED_BONUS * (1 - usedFraction);
  return { closeness, speedBonus, value: closeness + speedBonus, perColor };
}
