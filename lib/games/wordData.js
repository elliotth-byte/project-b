...
// Vibrant is the default look — matches the original artifact's palette.
// Colorblind-safe (Okabe-Ito) is available as a swap-in, same
// player-level toggle every other game respects (player.gamePrefs.
// colorBlindMode — see lib/gamePrefs.js), not a permanent replacement.
export const WORD_COLORS_VIBRANT = ["#ff6b6b", "#ffd93d", "#6bcb77", "#4d96ff", "#c879ff", "#ff9f4d", "#4de0d0"];
export const WORD_COLORS_SAFE = COLOR_BLIND_SAFE_PALETTE;
export function wordColorsFor(colorBlindMode) {
  return colorBlindMode ? WORD_COLORS_SAFE : WORD_COLORS_VIBRANT;
}
  ...
  words.forEach((word, wi) => {
    for (const char of word) {
      letters.push({
        ...
        // Slowed down twice now based on player feedback: originally
        // 1800-3200ms, then 3000-5000ms, now roughly doubled again since
        // it was still coming through too fast to actually read a letter
        // in the moment it's visible.
