...
// Scoped by round AND the specific challenge instance (its own
// startedAt), NOT round alone — this is the fix for a real, confirmed
// production bug: round-only scoping meant that if a SECOND, different
// challenge attempt ever started within the same round number (a
// reroll, a re-pick after the host changed their mind, any scenario
// where challenge.startedAt gets a new value without the round number
// itself incrementing), a game querying this for the new attempt would
// silently inherit a leftover timestamp from whatever ran earlier in
// that same round — even a completely different mini-game — producing
// a wildly wrong (and, once the fix in RedLightGreenLightPlayer.jsx and
// friends floors the resulting score, actually IMPOSSIBLE-looking:
// this predates that floor entirely, so it showed up as raw, huge
// negative numbers) elapsed time. challengeStartedAt is a required
// parameter specifically so this can't quietly regress the same way if
// a new caller is added later without thinking about it.
const sessionKey = (round, challengeStartedAt) => `pb:challenge-session:${round}:${challengeStartedAt}`;
...
// for a given player+round+challenge-instance wins and is persisted; every
// later call (including from a brand new mount) gets that same timestamp
// back — but a genuinely NEW challenge instance (different startedAt) always
// gets a genuinely fresh session, never a leftover one from something else
// that happened to run in the same round.
export async function getOrStartSession(gameId, round, challengeStartedAt, playerId) {
  const res = await storageUpdate(gameId, sessionKey(round, challengeStartedAt), (fresh) => {
    const existing = fresh || {};
...
export async function peekSession(gameId, round, challengeStartedAt, playerId) {
  const value = await storageGet(gameId, sessionKey(round, challengeStartedAt));
...
// game anyway.
export async function resetPlayerAttempt(gameId, round, challengeStartedAt, playerId) {
  const scoresRes = await storageUpdate(gameId, scoresKey(round), (fresh) => {
    const existing = { ...(fresh || {}) };
    delete existing[playerId];
    return existing;
  });
  const sessionRes = await storageUpdate(gameId, sessionKey(round, challengeStartedAt), (fresh) => {
    const existing = { ...(fresh || {}) };
    delete existing[playerId];
    return existing;
  });
...
export function scoresToPlacements(scores, participants, rankDirection) {
  const rows = (participants || []).map((p) => {
    ...
    return {
      ...
      // Difference's "found" count, or a finisher's real elapsed time
      // for a game whose ranking `value` isn't itself real time —
      // see formatPlacementValue below) — never used for ranking,
      // which is always decided by `value` above.
      foundCount: s && !forfeited ? s.foundCount ?? null : null,
      timeMs: s && !forfeited ? s.timeMs ?? null : null,
  ...
  return [...played, ...didNotPlay].map((r, i) => ({
    playerId: r.playerId, name: r.name, place: i + 1, forfeited: r.forfeited, value: r.value, foundCount: r.foundCount, timeMs: r.timeMs, hostOverridden: r.hostOverridden,
  }));
}

// Games whose ranking `value` is really just an internal placement
// sentinel (a big number per finish tier, per lib/games/*Data.js's own
// placementValue comments) rather than anything resembling real
// points — showing it as "N pts" is actively misleading, and the row's
// own "#place" already says everything there is to say about how they
// did. Nothing else needs a label here at all.
const PLACEMENT_ONLY_GAMES = new Set(["musicalchairs", "torched"]);

// A game-specific noun for a did-not-finish row's raw progress value,
// for score-desc games whose "ran out of time" blurb explicitly ranks
// by partial progress (Colorful Language, Sliding Puzzle, Scavenger
// Hunt) — plain "N pts" undersold what that number actually was.
// Scavenger Hunt's own placementValue packs two numbers into one (see
// its own comment: distinctTypes*1000 + inventory.length), unpacked
// here for display only.
const DNF_PROGRESS_LABELS = {
  stroop: (v) => `${v} cleared`,
  slidingpuzzle: (v) => `${v} tiles placed`,
  scavengerhunt: (v) => `${Math.floor(v / 1000)}/8 offerings`,
};
...
export function formatPlacementValue(placement, gameType, rankDirection) {
  ...
  // A finisher's real elapsed time, when the game reported one (see
  // scoresToPlacements above) — always preferred over the raw ranking
  // value, which for some of these games is itself just a big
  // finish-tier number engineered for sorting, not a real duration.
  if (placement.timeMs != null) {
    const formatted = `${(placement.timeMs / 1000).toFixed(2)}s`;
    return placement.hostOverridden ? `${formatted} (edited)` : formatted;
  }
  if (placement.foundCount != null) return `${placement.foundCount} found`;
  if (PLACEMENT_ONLY_GAMES.has(gameType)) return null;
  if (placement.value == null) return null;
  const dnfLabel = DNF_PROGRESS_LABELS[gameType];
  const formatted = rankDirection === "time-asc"
    ? `${(placement.value / 1000).toFixed(2)}s`
    : dnfLabel ? dnfLabel(placement.value) : `${placement.value} pts`;
  return placement.hostOverridden ? `${formatted} (edited)` : formatted;
}
