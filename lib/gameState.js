...
export const KEY_UNLOCKED_GAMES = "pb:unlocked-games"; // season-wide, ever-growing array of gameTypes seen so far (played, or offered as a Hephaestus option) — see lib/unlockedGames.js
...
export const DEFAULT_SETTINGS = {
  ...
  // ─── Big Screen Mode ───
  // A whole different physical setup from the normal one-phone-per-
  // player game: everyone's together in the same room, Jackbox-style,
  // with a shared TV/monitor showing ceremonies and battle visuals
  // while phones become controllers. Off by default and meant to be
  // set once, at season creation — see components/AdminHost.jsx's own
  // toggle and pages/display.jsx (the actual TV-facing page a host
  // opens on the shared screen) for the rest of this infrastructure.
  // Deliberately a season-wide setting, not something that flips
  // mid-season: unlike chatEnabled, there's no safe way to introduce a
  // "now go check the TV instead of your phone" expectation partway
  // through a season already running the normal way.
  bigScreenMode: false,
