...
// `db` adapter — same feed, tagged "system" so it renders the same way
// an automated update does, just not actually generated from a game
// state transition the way makeInAppPostMessage's calls are.
export async function postSystemAnnouncement(gameId, text) {
  const clean = (text || "").trim();
  if (!clean) return { ok: false };
  const res = await storageUpdate(gameId, KEY_ANNOUNCEMENTS, (fresh) => {
    const list = fresh || [];
    return [...list, { text: clean, at: Date.now(), from: "system" }].slice(-MAX_ANNOUNCEMENTS);
  });
  return { ok: res.ok };
}
