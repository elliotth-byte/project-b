...
//
// "Active" means not yet ended, used by fetchActiveGames below;
// fetchMyGames (further down) returns every game regardless, for
// callers that need access to a FINISHED season too, not just a live
// one. The two game types' finale keys don't
...
// Every game this account can reach (hosted or played), each tagged
// with whether it's still active — the shared lookup both
// fetchActiveGames (below) and fetchMyGames build on.
async function resolveMyGames(userId) {
  ...
  for (const game of candidates.values()) {
    results.push({ gameId: game.id, name: game.name || "Untitled Season", gameType: game.game_type, active: await isGameActive(game) });
  }
  return results;
}

// Active only — what pages/notifications.jsx uses, since a per-game
// push-notification setting only makes sense for a season that's still
// actually running; a completed season's game has nothing left to
// notify about.
export async function fetchActiveGames(userId) {
  const games = await resolveMyGames(userId);
  return games.filter((g) => g.active).map(({ active, ...g }) => g);
}

// Every game, active or not — pages/index.jsx's own use: a completed
// season's chat, recap, and Ceremony/History tabs are still there
// (pages/play.jsx keeps serving all of that once round.phase is ENDED,
// it only stops showing the live "game" tab), so a player who's done
// playing shouldn't lose their way back in, just the "active" framing
// of the link. fetchActiveGames itself deliberately keeps EXCLUDING
// ended seasons — that behavior is correct for notifications, just not
// for "can I still get back into a season I already finished," which
// is a different question this answers instead.
export async function fetchMyGames(userId) {
  return resolveMyGames(userId);
}
