...
The Favor of the Fates is also up for grabs this round (see below) — whoever draws it can secretly protect one nominee from being exiled no matter how many votes they get.

If there's a tie for the most votes, the Favor of the Fates holder breaks it.`,
  },
  {
    title: "🃏 The Favor of the Fates",
    body: `Once per Exile Vote (and once per Finale vote), every eligible player gets a single shot at secretly drawing the Favor of the Fates — a set of mythological relics (Pandora's Box, Eris' Apple, Troy's Horse, and others), one of which is secretly the "correct" one. Whoever draws it becomes the holder for that vote.
...
Everyone votes for which nominee to save. The nominee with the fewest save-votes is eliminated — and if the Favor of the Fates holder protects a different nominee that round, that protected nominee's votes are wiped to zero, guaranteeing THEY'RE also eliminated. Two people go home instead of one.`,
  },
  {
    title: "🏆 The Finale",
...
The Favor of the Fates is up for grabs among the jury too — the holder can wipe one finalist's votes entirely, guaranteeing they finish no better than 3rd regardless of how they actually polled.`,
  },
  {
    title: "🎥 Confessionals",
...
//
// unlockedTypes, when passed, restricts this to only the game types
// this SEASON has actually surfaced so far (played, or offered to
// Hephaestus — see lib/unlockedGames.js) rather than spoiling the
// entire registry up front. Omitted entirely (null/undefined) means no
// filtering at all — used by callers with no season context of their
// own (Practice Mode's own game picker, say), where showing the full
// registry is the actual intent, not an oversight.
export function battleList(unlockedTypes = null) {
  return Object.values(GAME_TYPES)
    .filter((type) => type !== GAME_TYPES.MANUAL)
    .filter((type) => !unlockedTypes || unlockedTypes.has(type))
