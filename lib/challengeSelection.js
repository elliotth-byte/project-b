...
export function eligibleGameTypes(challengeHistory) {
  const counts = categoryPlayCounts(challengeHistory);
  const played = playedGameTypes(challengeHistory);
  const allTypes = Object.keys(GAME_REGISTRY).filter((k) => k !== "manual");
...
export function pickRandomChallenge(challengeHistory) {
  const pool = eligibleGameTypes(challengeHistory);
...
export function pickHephaestusOptions(challengeHistory) {
  const pool = eligibleGameTypes(challengeHistory);
