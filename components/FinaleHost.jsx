...
export default function FinaleHost({ gameId, players, round }) {
  ...
  const buildFinaleSummary = () => {
    ...
    const chaosLine = nullifiedId ? `🃏 ${chaosHolder?.display_name || "The Favor of the Fates"} nullified ${byId[nullifiedId] || "?"} — they can't win.${chaosSecret?.reason ? ` ("${chaosSecret.reason}")` : ""}` : "";
    ...
    <Card style={{ borderColor: "rgba(255,45,149,0.4)" }}>
      ...
      <div style={{ background: "#0d0618", borderRadius: 8, padding: 10, marginBottom: 12, marginTop: 12 }}>
        <div style={{ fontSize: 11, color: "#a68fd6", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 6 }}>
          🃏 Favor of the Fates (drawn from the exiled)
        </div>
        {chaosHolder ? (
          <p style={{ fontSize: 12, color: nullifiedId ? "#00ff9d" : "#a68fd6", margin: 0 }}>
            ...
            {nullifiedId ? (
              <>✓ Locked in: nullifying <strong style={{ color: "#ff3860" }}>{byId[nullifiedId] || "?"}</strong> — visible to you now, still secret from players until the reveal.</>
            ) : (
          ...
          <p style={{ fontSize: 12, color: "#a68fd6", margin: 0 }}>
            Every exiled player got one shot at a mystery-card draw on their own screen ({exiledPlayers.length} cards, one Favor of the Fates) — {Object.keys(drawPicks).length}/{exiledPlayers.length} have picked so far.
        ...
        <Card style={{ borderColor: finale.tieBreakChoiceId ? "rgba(0,255,157,0.5)" : "rgba(255,56,96,0.5)", marginBottom: 12 }}>
          {finale.tieBreakChoiceId ? (
            ...
            <p style={{ color: "#ff3860", fontSize: 13, fontWeight: 700, margin: "0 0 8px" }}>
              🃏 It's tied — waiting on {chaosHolder?.display_name || "the Favor of the Fates holder"} to choose the winner from their own screen.
            </p>
          )}
