...
// Traitors-themed fork of ConfessionalPlayer.jsx — see
// TraitorsConfessionalsHost.jsx's comment for why this exists as its own
// file rather than a themeable prop. Same feature set, same shared
// confessionals table/data layer, Traitors' palette instead of Project
// B's neon one.
    ...
    <Card style={{ marginBottom: 20, borderColor: "rgba(124,58,237,0.35)", background: "linear-gradient(160deg, #150c1e 0%, #0e1830 100%)" }}>
      ...
      {prompts.length > 0 && (
        <div style={{ marginBottom: 12, display: "grid", gap: 6 }}>
          {prompts.map((p) => (
            <div key={p.id} style={{ background: "rgba(201,168,76,0.1)", border: "1px solid rgba(201,168,76,0.3)", borderRadius: 8, padding: "10px 12px" }}>
              <div style={{ fontSize: 10, color: "#c9a84c", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2 }}>
                {p.targetPlayerIds ? "A prompt just for you" : "Tonight's prompt"}
              </div>
              <div style={{ fontSize: 13, color: "#f0e6d3" }}>{p.prompt}</div>
            </div>
          ))}
        </div>
      )}
