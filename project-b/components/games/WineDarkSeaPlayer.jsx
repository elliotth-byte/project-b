...
import { subscribeWineDarkSea, submitPlacement, tickWineDarkSea, placementValue } from "../../lib/games/wineDarkSeaData";

// ─── The Wine-Dark Sea — Big Screen (phone side) ───
// Identical split as River Styx (see RiverStyxPlayer.jsx's own header)
// — the board, every ship, and Charybdis's whirlpools only ever show
// on the TV (components/bigscreen/WineDarkSeaTvDisplay.jsx). Your
// phone is purely your own hand of 3 wake tiles.
export default function WineDarkSeaPlayer({ gameId, round, player, players, settings }) {
  ...
  if (state.gameEnded) {
    ...
    const label = state.winnerIds.length > 1
      ...
      : iWon ? "Your ship made it home!" : "Your ship was lost at sea.";
    return <GameResultCard icon="🌀" title={iWon ? "Victory!" : "The Voyage Ends"} valueLabel={label} />;
  }
  ...
  if (myStatus === "eliminated") {
    ...
    return (
      <Card style={{ marginBottom: 20, textAlign: "center" }}>
        ...
        <h3 style={{ color: "#ff2d95", margin: "0 0 8px", fontSize: 15, fontFamily: "'Orbitron', 'Segoe UI', sans-serif" }}>Lost at Sea</h3>
        <p style={{ color: "#a68fd6", fontSize: 13 }}>
          You fell {fellAt >= 0 ? `${fellAt + 1}${["st", "nd", "rd"][fellAt] || "th"}` : ""} — watch the big screen for how the voyage ends.
        </p>
      </Card>
    );
  }
    ...
    return (
      <Card style={{ marginBottom: 20, textAlign: "center" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <h3 style={{ color: "#ff2d95", margin: 0, fontSize: 15, fontFamily: "'Orbitron', 'Segoe UI', sans-serif" }}>🌀 The Wine-Dark Sea</h3>
          <Badge>{activeCount} ships afloat</Badge>
        </div>
        ...
        <p style={{ color: "#6b4f99", fontSize: 11, marginTop: 10 }}>Your hand — ready for your turn. Watch for Charybdis!</p>
      </Card>
    );
  }
  ...
  return (
    <Card style={{ marginBottom: 20, textAlign: "center" }}>
      ...
      <p style={{ color: "#00ff9d", fontSize: 13, fontWeight: 700, margin: "0 0 12px" }}>Your turn — pick and lay a wake tile!</p>
      ...
      <Btn onClick={confirm} disabled={placing}>{placing ? "Laying tile..." : "🚢 Lay Wake Tile"}</Btn>
      <p style={{ color: "#6b4f99", fontSize: 11, marginTop: 10, fontStyle: "italic" }}>
        Look at the big screen — your ship follows wherever it's currently facing, and Charybdis may stir after.
      </p>
    </Card>
  );
}
