...
import { subscribeRiverStyx, tickRiverStyx, placementValue } from "../../lib/games/riverStyxData";
...
// ─── Big Screen: River Styx ───
// See lib/games/riverStyxData.js for the full mechanic — the shared
// 6x6 board, every shade's live position, and the placement log only
// ever show here. A phone (components/games/RiverStyxPlayer.jsx) only
// ever shows its own hand and a rotate/confirm control.
export default function RiverStyxTvDisplay({ gameId, round, players, settings }) {
  ...
  if (state.gameEnded) {
    ...
    return (
      <div style={{ padding: 40, textAlign: "center" }}>
        <div style={{ fontSize: 14, color: "#6b4f99", textTransform: "uppercase", letterSpacing: 3, marginBottom: 12 }}>💀 River Styx — The Crossing Ends</div>
        <p style={{ fontSize: 24, color: "#c9a84c", fontWeight: 800, fontFamily: "'Orbitron', 'Segoe UI', sans-serif", margin: "0 0 24px" }}>
          {state.winnerIds.length > 1
            ? `👑 Shared victory: ${state.winnerIds.map((id) => byId[id] || "?").join(", ")}`
            : `👑 ${byId[state.winnerIds[0]] || "?"} crosses the Styx unscathed!`}
        </p>
          ...
          {ranked.map((id) => (
            <div key={id} style={{
              ...
              <span style={{ color: colorForPlayer(state.participantIds, id), fontWeight: 800, fontSize: 15 }}>
                {state.winnerIds.includes(id) ? "Survived" : `Fell #${state.finishOrder.indexOf(id) + 1}`}
              </span>
            </div>
          ))}
        </div>
    ...
    <div style={{ padding: 32, display: "grid", gridTemplateColumns: `${BOARD_PX}px 1fr`, gap: 32, minHeight: "70vh", justifyContent: "center" }}>
      <div>
        <div style={{ textAlign: "center", marginBottom: 14 }}>
          <div style={{ fontSize: 13, color: "#6b4f99", textTransform: "uppercase", letterSpacing: 3 }}>💀 River Styx</div>
        ...
        <div style={{ position: "relative", width: BOARD_PX, height: BOARD_PX, margin: "0 auto" }}>
          ...
          {/* Marker overlay — positioned at each shade's exact resting edge point, not just centered on its cell. */}
      ...
      <div>
        <div style={{ fontSize: 13, color: "#6b4f99", textTransform: "uppercase", letterSpacing: 2, marginBottom: 10 }}>Shades</div>
          ...
          {state.participantIds.map((id) => {
            ...
            return (
              <div key={id} style={{
                ...
                <span style={{ fontSize: 12, color: "#a68fd6" }}>{alive ? `🎴 ${(state.hands[id] || []).length}` : "out"}</span>
              </div>
            );
          })}
        </div>
