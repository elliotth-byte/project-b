...
// Traitors-themed fork of ConfessionalsHost.jsx — same feature set (host
// replies, multi-target prompts, filters, recap builder, star/archive),
// just Traitors' brown/gold palette instead of Project B's neon one. See
// README.md's "Game types" section for why this exists as a separate
// file rather than a themeable prop: it matches the fork-under-a-
// Traitors-prefixed-name pattern already used for
// TraitorsAdminHost/TraitorsHistoryTab/TraitorsHostPanels, and the
// underlying confessionals table/data layer (lib/confessionalsData.js) is
// still fully shared — only the rendering differs.
    ...
    <div style={{ display: "grid", gap: 16 }}>
      <Card>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12 }}>
          <label style={{ fontSize: 10, color: "#706050", textTransform: "uppercase", letterSpacing: 0.5, whiteSpace: "nowrap" }}>Replying as</label>
          <input
            value={postAsName}
            onChange={(e) => setPostAsName(e.target.value)}
            placeholder="Host"
            maxLength={40}
            style={{ flex: 1, maxWidth: 220, background: "#1a1512", border: "1px solid #3a3028", borderRadius: 6, padding: "4px 8px", color: "#c9a84c", fontSize: 11 }}
          />
        </div>
        ...
        <div style={{ background: "#0a1020", borderRadius: 8, padding: 10, marginBottom: 12 }}>
          ...
          {prompts.length > 0 && (
            <div style={{ display: "grid", gap: 6, marginBottom: 10 }}>
              {prompts.map((p) => (
                <div key={p.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, background: "#132038", borderRadius: 6, padding: "6px 10px" }}>
                  <span style={{ fontSize: 12, color: "#f0e6d3" }}>
                    {p.prompt}
                    <span style={{ display: "block", fontSize: 10, color: p.targetPlayerIds ? "#c9a84c" : "#706050", marginTop: 2 }}>
          ...
          <input
            ...
            style={{ width: "100%", boxSizing: "border-box", background: "#132038", border: "1px solid #253550", borderRadius: 6, padding: "6px 10px", color: "#f0e6d3", fontSize: 12, marginBottom: 8 }}
          />
            ...
            {(players || []).map((p) => {
              ...
              return (
                <button key={p.id} onClick={() => togglePromptTarget(p.id)} style={{
                  ...
                  border: `1px solid ${selected ? "#c9a84c" : "#253550"}`,
                  color: selected ? "#c9a84c" : "#a09080",
                }}>{selected ? "✓ " : ""}{p.display_name}</button>
              );
            })}
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 11, color: "#706050", fontStyle: "italic" }}>
          ...
          <div style={{ display: "grid", gap: 4, marginBottom: 10, maxHeight: 200, overflowY: "auto" }}>
            {items.filter((c) => !c.archived).map((c) => (
              <label key={c.id} style={{ display: "flex", gap: 6, alignItems: "flex-start", fontSize: 12, color: "#a09080" }}>
                ...
                <span>{c.starred && "⭐ "}<strong style={{ color: "#f0e6d3" }}>{identityLabel(c)}</strong> — {c.text.slice(0, 80)}{c.text.length > 80 ? "..." : ""}</span>
              </label>
            ))}
          </div>
          ...
          <div style={{ display: "grid", gap: 8 }}>
            {groupItems.length === 0 ? (
              ...
              <Card key={c.id} style={{
                ...
                {compact ? (
                  ...
                  >
                    <span style={{ fontSize: 12, color: "#a09080" }}>
                      ...
                      <strong style={{ color: "#f0e6d3" }}>{identityLabel(c)}</strong>
                    ...
                    <span style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
                      ...
                      {c.starred && <span>⭐</span>}
                      {c.text.length > 70 && <span style={{ color: "#706050", fontSize: 10 }}>{expandedIds.has(c.id) ? "▲ collapse" : "▼ expand"}</span>}
                    </span>
                  </div>
                ) : (
                  <>
                ...
                <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid #253550" }}>
                    {c.host_reply && (
                      <div style={{ background: "rgba(74,122,196,0.08)", border: "1px solid rgba(74,122,196,0.3)", borderRadius: 6, padding: "6px 10px", marginBottom: 6 }}>
                        <div style={{ fontSize: 10, color: "#4a7ac4", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2 }}>Your private reply</div>
