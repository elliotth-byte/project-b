...
//    the fate nominees. Favor of the Fates nullifies all votes AGAINST its
...
// ─── Fallback tie-break: no Favor of the Fates holder to ask ───
// Used when a vote ties but there's nobody actually holding the Favor of
// the Fates this round (nobody won the mystery-card draw — see
// pages/api/chaos-draw.js), so the normal "wait for the holder's pick"
// path (resolveEliminateTie, above) can never resolve on its own. Falls
// back to whichever tied nominee did WORST in that same round's
// challenge (highest `place` — see lib/challengeScores.js), since
// that's a real, already-known, un-gameable signal for "who should go"
// that doesn't depend on any one player acting.
//
// challengePlacements is the round's `challenge.placements` array,
// already ranked (see advanceFromChallenge in lib/roundEngine.js):
// [{ playerId, place, ... }]. Only entries for the tied nominees are
// considered.
//
// Returns:
//  - a single playerId, if exactly one tied nominee had the single
//    worst (highest) recorded place among them.
//  - an array of playerIds, if two or more of the tied nominees shared
//    that worst place (still tied even by challenge score) — caller is
//    expected to break that remaining tie with pickRandomAmong.
//  - the full `tiedIds` array unchanged if NONE of the tied nominees
//    have any placement on record at all (e.g. they didn't participate
//    that round, or the challenge data is missing) — there's no
//    challenge-score signal to use at all in that case, so this
//    signals "can't help, still fully tied" rather than silently
//    picking someone with a fabricated worst-place value.
export function resolveEliminateTieByChallenge(tiedIds, challengePlacements) {
  const ids = tiedIds || [];
  const placementById = {};
  (challengePlacements || []).forEach((p) => {
    if (p && p.playerId != null) placementById[p.playerId] = p;
  });

  const withPlacement = ids.filter((id) => placementById[id] && typeof placementById[id].place === "number");
  if (withPlacement.length === 0) return [...ids];

  const worstPlace = Math.max(...withPlacement.map((id) => placementById[id].place));
  const worst = withPlacement.filter((id) => placementById[id].place === worstPlace);

  // Anyone with NO recorded placement at all didn't play — they can't be
  // judged worse or better by a score they don't have, so they're left
  // out of contention here entirely rather than treated as tied-worst.
  return worst.length === 1 ? worst[0] : worst;
}

// Trivial random-pick helper, shared by any tie-break fallback that
// needs to land on a single id among an already-fully-tied set.
export function pickRandomAmong(ids) {
  const list = ids || [];
...
// those two is broken by the exiled Favor of the Fates holder's own choice.
