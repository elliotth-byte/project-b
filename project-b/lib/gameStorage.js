...
// Every 6 seconds was needlessly aggressive for what this is actually
// guarding against — the visibility-change resync below already
// catches the single most common way a subscription goes stale (a tab
// being backgrounded and re-foregrounded) INSTANTLY, the moment it
// happens. This periodic poll only exists for the rarer case of
// realtime silently dying while the tab stays visible the whole time —
// a real but genuinely uncommon event, not one that needs sub-10-second
// detection. With 70+ call sites across this app, each one running its
// own independent poll, 6 seconds meant a large, constant background
// request volume for every open tab regardless of whether anything was
// actually happening — a real, confirmed contributor to hitting a
// Supabase egress cap. 45 seconds still means any missed-event
// staleness clears on its own well within a minute, which is more than
// fast enough for a turn-based game, while cutting this specific
// request volume by roughly 7x.
