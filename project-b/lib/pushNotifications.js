...
// ─── Player-facing round-change notification, from the browser ───
// Project B's own round-change notifications (lib/roundEngine.js's
// notifyRoundChange) fire from server code that already runs inside an
// API route/the cron job, so they call lib/sendPush.js's
// sendPushToGame directly — no HTTP hop needed. Traitors and Stereo
// Types trigger their round changes from the BROWSER instead (a host's
// click, or an opportunistic client-side effect), which can't hold the
// private VAPID key, so this goes through
// pages/api/push/notify-round-change.js the same way
// notifyHostsForConfessional (components/ConfessionalPlayer.jsx) already
// proxies its own event through notify-host-event.js. Fire-and-forget —
// a notification failing to send should never surface as an error for
// whatever actual game action just succeeded.
//
// playerIds (optional) targets a specific subset instead of the whole
// game — see that API route's own comment on why
// components/MurderVoteHost.jsx needs this rather than a broadcast.
export async function notifyPlayersRoundChange(gameId, title, body, tag, playerIds) {
  try {
    ...
    await fetch("/api/push/notify-round-change", {
      ...
      body: JSON.stringify({ gameId, title, body, tag, playerIds }),
    });
  } catch (e) {
...
// ─── Chat-message push notification, from the browser ───
// Extracted from components/ChatPanel.jsx's own local copy (previously
// named notifyPushForMessage there) so components/FinalWordsPrompt.jsx
// can trigger the exact same push for a Final Words message too — sent
// through lib/finalWords.js's submitFinalWords, which writes straight
// to the group chat's own storage rather than going through
// ChatPanel.jsx's onSend at all, so it never triggered a push of any
// kind before this. Fire-and-forget, same reasoning as
// notifyPlayersRoundChange above: a push failing to send is never
// worth surfacing as an error for a message that already sent
// successfully.
//
// isFinalWords (optional) tells pages/api/push/notify-message.js's own
// "group" handler to title the notification as Final Words rather than
// a normal chat message — see that route's own comment.
export async function notifyPushForMessage(gameId, kind, senderId, senderName, body, threadId, isFinalWords) {
  try {
    const { data } = await supabase.auth.getSession();
    const token = data?.session?.access_token;
    if (!token) return;
    await fetch("/api/push/notify-message", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      body: JSON.stringify({ gameId, kind, senderId, senderName, body, threadId, isFinalWords }),
    });
  } catch (e) {
