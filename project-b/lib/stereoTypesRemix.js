...
export async function startRemix(gameId, approvedPlayers) {
  ...
  // justStarted is only ever true for the ONE caller whose write actually
  // created this round — every other concurrent caller (this function
  // runs opportunistically from every connected client, host and
  // players alike, once Round 1 finishes scoring) sees `fresh` already
  // set and returns early above, so this is a safe way for a caller to
  // know "was I the one who just triggered this," worth relying on
  // for anything that should only ever happen ONCE per real transition
  // — see StereoTypesASideHost.jsx/StereoTypesASidePlayer.jsx's own use
  // of this for the round-change push notification, which would
  // otherwise fire once per connected client instead of once per game.
  return { ok: res.ok, justStarted };
}
