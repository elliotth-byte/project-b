...
export function resolveIdentities(players, { settings, round, isHost, revealed }) {
  ...
  return list.map((p) => ({
    ...
    // Falls back to a neutral "still choosing" placeholder if this
    // specific player hasn't actually chosen an alias yet (mid-
    // approval, or joined before the toggle was turned on) — NOT to
    // their real name. That used to be the fallback here, on the
    // reasoning that a blank identity is worse than something — true,
    // but the real name isn't "something," it's the exact thing every
    // other player on this screen is specifically not able to see
    // about EACH OTHER. Falling back to it just because this one
    // player is running behind on picking a character leaked their
    // identity for a reason that has nothing to do with whether it's
    // supposed to be secret — same gap sql/fix-season-roster-pending-alias.sql
    // fixes for the Season Roster; this is the same bug in the other
    // place player identity gets resolved (chat, the in-game Memory
    // Wall, everywhere resolveIdentities feeds).
    display_name: p.alias || "Choosing character...",
  }));
}
