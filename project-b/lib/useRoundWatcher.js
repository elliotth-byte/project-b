...
//
// `enabled` exists for Traitors seasons specifically: they never write
// Project B's own round-phase key at all (see lib/gameState.js), so
// every call this hook makes for one is a guaranteed no-op round trip,
// forever — not worth the egress on a screen that will never advance a
// phase. host.jsx/play.jsx pass enabled: !isTraitors.
export function useRoundWatcher(gameId, { intervalMs = 10000, enabled = true } = {}) {
