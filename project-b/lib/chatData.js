...
import { checkMessage } from "./contentFilter";
...
export async function sendGroupMessage(gameId, senderId, senderName, body, realName) {
  ...
  if (!filterResult.allowed) return { ok: false, error: filterResult.reason };
...
// ─── Reporting ───
// See sql/add-chat-reports-and-blocking.sql — mirrors dm_reports'
// exact shape (one report per specific message, a reason, reviewed by
// a platform admin later) applied to in-game chat instead of the
// cross-season profile DM system that was the only place this existed
// before.
export async function reportChatMessage(messageId, reporterId, reason) {
  const trimmed = (reason || "").trim();
  if (!trimmed) return { ok: false, error: "A reason is required." };
  const { error } = await supabase.from("chat_message_reports").insert({ message_id: messageId, reporter_id: reporterId, reason: trimmed.slice(0, 500) });
  return { ok: !error, error: error?.message };
}
