...
// Locking in your own chain doesn't reveal anything about the outcome
// — that genuinely can't be known until every participant has locked
// in and the server-side reveal actually runs (see submitChain) — but
// it IS real, honest progress, and worth distinguishing from never
// having locked in at all if the challenge's own timer runs out before
// the reveal ever happens. Both interim tiers (1 for locked in, 0 for
// never locked in) stay far below anything a genuine revealed result
// can produce: the smallest possible revealed score already multiplies
// by 1e13, so even a 0-point loss on every pairing still outranks
// either interim value by many orders of magnitude — there's no way
// for "made progress but the clock ran out" to accidentally beat an
// actual result.
export function placementValue(state, playerId) {
  if (!state.revealed) return state.chains?.[playerId] ? 1 : 0;
