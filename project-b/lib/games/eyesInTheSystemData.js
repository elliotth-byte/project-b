...
// The three zones' target-color counts: a genuinely close call rather
// than an at-a-glance landslide — the highest of the three lands
// between 15 and 25, and each step down from it is a tight 1-3 margin,
// so telling zones apart takes real counting. Built top-down (highest,
// then two margins subtracted off it) rather than three independent
// rolls specifically so those margins are guaranteed small; shifted up
// afterward if that ever pushes the lowest below 15 (the worst case —
// highest 20, both margins maxed at 3 — lands exactly on 14, one below
// the floor, so this only ever needs a 1-point nudge). Still always
// three distinct values (a margin is always >= 1), so the "never tied"
// guarantee generateRound's own header comment describes still holds.
function pickTargetCounts(rng) {
  const highest = 20 + Math.floor(rng() * 6); // 20..25
  const marginA = 1 + Math.floor(rng() * 3); // 1..3
  const marginB = 1 + Math.floor(rng() * 3); // 1..3
  let c1 = highest, c2 = highest - marginA, c3 = highest - marginA - marginB;
  if (c3 < 15) {
    const shift = 15 - c3;
    c1 += shift; c2 += shift; c3 += shift;
  }
  return shuffle([c1, c2, c3], rng);
}
...
export function generateRound(rng) {
  ...
  const targetCountPool = pickTargetCounts(rng);
