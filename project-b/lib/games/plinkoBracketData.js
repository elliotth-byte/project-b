...
// db: optional { set, update, ... } override — see lib/dbAdapter.js's
// makeDb, which is exactly what lets this same logic run bound to
// either the browser's client (existing callers, e.g.
// components/ChallengeHost.jsx, which never pass this) or the
// service-role client the server-side round engine actually uses
// (lib/roundEngine.js's own env.db, passed through when a challenge
// auto-starts unattended). Calling storageSet/storageUpdate directly
// from a server context would silently fail — those are bound
// specifically to the browser client and have no valid session there.
export async function initPlinkoBracket(gameId, round, participants, seed, db) {
  ...
  await set(gameId, bracketKey(round), {
    totalPlayers: total,
