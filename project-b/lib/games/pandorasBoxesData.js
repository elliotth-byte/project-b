...
// Same interim-tier idea as lib/games/chainsData.js's own
// placementValue (see its comment for the full reasoning on why this
// matters and why these interim values can never accidentally outrank
// a real revealed result — the same 1e13 scaling applies here too).
// Two tiers rather than Chains' one, matching this game's own two
// rounds: having locked in round 2 (or had nothing left to give away,
// which round1Gifts alone can't distinguish from "hasn't gotten to it
// yet" — an acceptable simplification for what's meant to be a rare
// fallback, not the normal path) ranks above only having locked in
// round 1, which ranks above having done nothing at all.
export function placementValue(state, playerId) {
  if (state.phase !== "revealed") {
    if (state.round2Gifts?.[playerId]) return 2;
    if (state.round1Gifts?.[playerId]) return 1;
    return 0;
  }
