...
export function generateScenes(seed, diffCount, W, H) {
  ...
  // Scale shape density with canvas area so a bigger board doesn't just
  // look sparse — 14 shapes was tuned for the original 300x200 board.
  const shapeCount = Math.max(14, Math.round(14 * (W * H) / (300 * 200)));
  const shapes = [];
...
// Pure rendering upgrade (tier-2 graphics pass) — every shape now gets a
// radial gradient + soft drop shadow instead of one flat fill color.
// generateScenes' own shape positions/colors/mutations (the actual game
// logic — what makes two scenes differ) are completely untouched; this
// only changes how a given shape is painted once its data already says
// what it is.
...
// Lightens a "#rrggbb" hex color toward white, for the gradient
// highlight above — purely a rendering helper, not used anywhere in the
// actual scene/difference generation logic.
