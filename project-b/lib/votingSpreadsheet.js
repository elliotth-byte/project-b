...
export function buildVotingGrid(exileHistory, finaleState, players, challengeHistory) {
  ...
  // Favor of the Fates holder zeroed out every vote cast for that target that
  ...
  const playerRows = orderedIds.map((id) => ({
    ...
    // cell always carries these flags; only `target` (and `nullified`,
    // which only ever applies to a vote THEY cast) is null when there's
    // no vote to show. savedByFates takes priority in the component's
    // own color choice below over plain "nominated" — being nominated
    // but having your votes zeroed out is a meaningfully different
    // outcome from an ordinary nomination.
    cells: columns.map((c) => {
      const vote = votesByColumn[c.key]?.[id] ?? null;
      ...
      return { target: vote?.target ?? null, nullified: vote?.nullified || false, nominated, immune, savedByFates };
    }),
  }));
...
export function votingGridToCSV(grid) {
  ...
  const lines = grid.playerRows.map((r) => [
    ...
    ...r.cells.map((c) => (c?.target ? c.target + (c.nullified ? " (nullified)" : "") : "")),
  ].map(csvEscape).join(","));
  return [
    header,
