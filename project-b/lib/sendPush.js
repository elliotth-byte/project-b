...
// arrived).
export async function sendPushToGame(gameId, { title, body, url, tag, filterColumn, excludePlayerId, aliveOnly }) {
  ...
  // Scoped, opt-in only — most callers of this function (round-change
  // announcements, for one) should still reach an exiled player who's
  // still watching the season, so this is never the default. Currently
  // only the main group chat notification actually asks for it — see
  // pages/api/push/notify-message.js's own comment on why chat pings
  // specifically shouldn't keep reaching someone once they're out.
  if (aliveOnly) {
    const { data: alive } = await adminClient.from("players").select("id").eq("game_id", gameId).eq("alive", true);
    const aliveIds = (alive || []).map((p) => p.id);
    if (aliveIds.length === 0) return; // nobody left to notify — avoid relying on an empty .in() array's edge-case behavior
    query = query.in("player_id", aliveIds);
  }
...
// Host-side counterpart — sends to every host (primary or co-) on this
// game who's subscribed with the given preference flag on. filterColumn
// is one of host_push_subscriptions' own columns: "notify_new_confessional"
// or "notify_pending_player" (see sql/add-host-push-subscriptions.sql).
export async function sendPushToHosts(gameId, { title, body, url, tag, filterColumn }) {
  if (!ensureConfigured()) return;

  const adminClient = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
  const { data: subs, error } = await adminClient
    .from("host_push_subscriptions").select("*").eq("game_id", gameId).eq(filterColumn, true);
  if (error || !subs || subs.length === 0) return;
  ...
  await Promise.all(subs.map(async (sub) => {
    ...
    try {
      ...
      if (err.statusCode === 404 || err.statusCode === 410) staleIds.push(sub.id);
      else console.error("Host push send failed:", sub.id, err.statusCode, err.body);
    }
  }));

  if (staleIds.length > 0) {
    await adminClient.from("host_push_subscriptions").delete().in("id", staleIds);
  }
}
