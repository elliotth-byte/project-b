...
export async function quitOrRemoveApprovedPlayer(gameId, playerId, eliminationRound) {
  const res = await supabase.from("players").update({ alive: false, elimination_type: "quit", elimination_round: eliminationRound ?? null }).eq("id", playerId);
  if (!res.error) await recordElimination(supabase, gameId, playerId);
  return res;
}
