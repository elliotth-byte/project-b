...
import { catalogByKey } from "./achievements/catalog";
...
// Achievements are public-read directly off player_achievements (see
// sql/add-achievements.sql) — no security-definer RPC needed the way
// fetchSeasonHistory/searchSeasons require, since the table's own RLS
// already allows anyone to read anyone's earned achievements straight
// up. Joins each earned row against the current catalog
// (lib/achievements/catalog.js) so a caller gets the full display
// info (name/icon/description), not just the bare key that got
// stored — catalog entries are never persisted redundantly in the
// table itself, so this join is what makes an earned row actually
// displayable.
export async function fetchAchievements(userId) {
  const { data, error } = await supabase
    .from("player_achievements")
    .select("achievement_key, game_id, earned_at")
    .eq("user_id", userId)
  ...
  return data
    .map((row) => {
      const meta = byKey[row.achievement_key];
      if (!meta) return null; // a key from a since-retired achievement — skip rather than show a blank entry
      return { ...meta, gameId: row.game_id, earnedAt: row.earned_at };
    })
    .filter(Boolean);
}
