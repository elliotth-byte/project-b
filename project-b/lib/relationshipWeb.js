...
import { fetchProfile, fetchSeasonHistory, fetchSeasonRoster, fetchMostRecentAvatars } from "./profiles";
...
export async function fetchRelationshipWeb(subjectUserId) {
  ...
  // is enough to build the ring; the portrait itself prefers their
  // persistent PROFILE photo (bulk-fetched below) so the same face shows
  // up consistently wherever they appear, falling back to their own most
  // recent season's avatar (fillMissingAvatars, below) only when they've
  // never set a profile photo at all.
  ...
  await fillMissingAvatars([me, ...innerRing, ...outerRing]);
...
// Mutates each member's own .photoUrl in place — filled from their own
// most recent season's avatar (see lib/profiles.js's
// fetchMostRecentAvatars) wherever a profiles.photo_url wasn't already
// set. Batched: one call for everyone missing a photo across the whole
// diagram (me + both rings), never one call per member.
async function fillMissingAvatars(members) {
  const missingIds = [...new Set(members.filter((m) => !m.photoUrl).map((m) => m.userId))];
  if (missingIds.length === 0) return;
  const fallbacks = await fetchMostRecentAvatars(missingIds);
  members.forEach((m) => { if (!m.photoUrl) m.photoUrl = fallbacks[m.userId] || null; });
}
