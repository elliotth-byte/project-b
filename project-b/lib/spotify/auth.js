// ─── Spotify OAuth — PKCE, one connection per browser tab ───
// Originally host-only; now any browser tab can authenticate this way
// — the host to run the boombox, or an individual player who wants the
// music to play through their OWN device too (see
// StereoTypesPlayerSpotifySync.jsx). Nothing below cares which: this
// module just runs the standard PKCE flow (RFC 7636) entirely in
// whichever browser tab calls it, with no server code and no client
// secret — PKCE exists specifically so a public client like this one
...
// only — never written to Supabase, never visible to anyone else's
// browser — which also means closing the tab means reconnecting next
// time; that's an intentional trade for "no Spotify credential of any
// kind ever touches our database," host or player alike.
//
...
// state round-trips gameId (and which kind of screen to bounce back
// to) through Spotify's own redirect (which only ever carries
// `code`/`state`/`error`) so the callback page knows where to send this
// browser once the exchange finishes — base64url-encoded JSON, not a
// Spotify-issued value, so decoding it never needs a network round trip.
...
// role defaults to "host" (the original, only caller) — a player
// connecting their own account for playback sync passes "player" so
// the callback bounces back to /play instead of /host.
export async function beginAuth(gameId, role = "host") {
