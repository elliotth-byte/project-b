...
//
// That later phase landed: see getSuperlativePool below.
//
...
// Plain read, no {ok, error} wrapper — same shape lib/profiles.js's own
// simple fetch-and-map functions use, and the same shape
// fetchUnlockedStickerIds in lib/stereoTypesStickers.js already picked
// for "a failure here just means nothing extra to show," not something
// worth a caller branching on. RLS (see
// sql/add-stereo-types-superlative-submissions.sql) only lets a
// platform admin SELECT this table at all — a regular player's client
// gets zero rows back for an approved row it didn't write itself, same
// as anyone else's; that's fine, because approved text isn't sensitive
// and the whole point of this function is to hand it back to EVERY
// player anyway, dealt out as part of the pool below.
async function fetchApprovedSubmissionTexts() {
...
// The one function every round's dealing logic should call instead of
// importing SUPERLATIVES directly (see this file's own header comment —
// this is the extension point it was written to anticipate). Appends
// approved user submissions to the static seed list at load time, exactly
// as that comment described. Falls back to the static list alone if the
// fetch itself fails for any reason (RLS not yet migrated, a network
// blip, ...) — a broken or empty submissions table should never be able
// to take down a round that would otherwise deal fine from the seeded
// pool by itself.
export async function getSuperlativePool() {
  const approved = await fetchApprovedSubmissionTexts();
  return [...SUPERLATIVES, ...approved];
}

// Text -> submitter display name, for approved player-submitted
// superlatives only (the seeded SUPERLATIVES list has no submitter at
// all, so it's simply absent from this map — a lookup miss means "not a
// player submission," not an error). Purely a display-time concern: the
// dealing functions (dealSuperlatives et al., in lib/stereoTypesASide.js
// and its Remix/On Blast counterparts) still only ever carry the plain
// text forward into game_state, same as before this existed — this is
// meant to be called separately, wherever a superlative is actually
// RENDERED to a player, so a reveal card can show "— submitted by
// Jordan" alongside it. Keyed by body text (not submission id) since
// text is all that survives into game_state once something's dealt out.
// A name collision between two different approved submissions sharing
// identical text is possible but harmless here (same "not a correctness
// problem" judgment call sql/add-stereo-types-superlative-submissions.sql
// already makes about duplicate text) — whichever one this map lands on
// last just wins the display credit.
export async function getSuperlativeAttributions() {
  const { data, error } = await supabase
    .from("stereo_types_superlative_submissions")
    .select("body, user_id")
    .eq("status", "approved");
  ...
  const namesById = {};
  (profs || []).forEach((p) => { namesById[p.user_id] = p.display_name; });

  const map = {};
  data.forEach((row) => {
    const name = namesById[row.user_id];
    if (name) map[row.body] = name;
  });
  return map;
}
